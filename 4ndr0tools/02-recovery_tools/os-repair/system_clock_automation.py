...
def create_system_user(
    ...
    try:
        ...
        result = subprocess.run(
            ["getent", "group", user], capture_output=True, text=True
        )
        ...
        if groups:
            for group in groups:
                result = subprocess.run(
                    ["getent", "group", group], capture_output=True, text=True
                )
                if result.returncode != 0:
...
def check_openssl_version(required_version):
    ...
    try:
        ...
        if current_version != required_version:
            logging.warning(
                f"OpenSSL version mismatch. Required: {required_version}, Found: {current_version}"
            )
...
def main():
    ...
    users = [
        ...
        {"user": "ntp", "uid": 87, "gid": 87, "desc": "Network Time Protocol"},
        {
            ...
            "uid": 954,
            "gid": 954,
            "desc": "Redis in-memory data structure store",
        },
        {
            "user": "rtkit",
            "uid": 133,
