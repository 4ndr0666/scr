...
[[ -r "$common" ]] || {
    printf '[FAIL] common.sh not readable: %s\n' "$common" >&2
    exit 1
}
...
[[ -s "$unit_file" ]] || {
    printf '[FAIL] Could not extract run_bounded() from %s\n' "$common" >&2
    exit 1
}

grep -q '^run_bounded()' "$unit_file"
grep -q '^}$' "$unit_file"
...
child_status=0
bash --noprofile --norc -s "$unit_file" <<'CHILD' &
set -euo pipefail
IFS=$'\n\t'
...
if run_case 'ordinary failure preserves rc' 7 run_bounded 5 'exit-7' bash -c 'exit 7'; then ((pass+=1)); else ((fail+=1)); fi
if run_case 'timeout returns 124' 124 run_bounded 1 'sleep-timeout' sleep 5; then ((pass+=1)); else ((fail+=1)); fi
...
if run_case 'TERM-resistant process reaches KILL' 137 run_bounded 1 'term-resistant' bash -c 'trap "" TERM; sleep 30'; then ((pass+=1)); else ((fail+=1)); fi
...
printf '[PASS] Isolated run_bounded() runtime proof\n'
