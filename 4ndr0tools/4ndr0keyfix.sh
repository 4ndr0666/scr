...
removedb() {
	...
	sleep 1
	rm -rf /var/lib/pacman/sync/* || glow "${EXPLOSION} failed to remove databases"

	glow "Removing corrupt gpg files..."
	sleep 1
	rm -rf /etc/pacman.d/gnupg/* || glow "${EXPLOSION} failed to remove /etc/pacman.d/gnupg files"
}
...
initdb() {
	...
	pacman-key --init || glow "Failed to execute Pacman-key --init."
	...
	pacman-key --lsign-key 3056513887B78AEB || glow "Failed to sign keys..."
...
syncdb() {
	...
	pacman -Sy && pacman -Sy archlinux-keyring --noconfirm || glow "Failed to reinstall archlinux-keyring."
}
