...
def process_dep_scan_log():
    ...
    try:
        with spinning_spinner():
            ...
            if missing_deps:
                log_and_print(
                    f"{INFO} Installing missing dependencies in batch: {list(set(missing_deps))}",
                    "info",
                )
...
def remove_broken_symlinks():
    ...
    try:
        ...
        broken_links = [
            link for link in result.stdout.strip().split("\n") if link.strip()
        ]
        if not broken_links:
    ...
    with spinning_spinner():
        try:
            explicitly_installed = (
                subprocess.check_output(["pacman", "-Qeq"], text=True)
                .strip()
                .split("\n")
            )
            ...
            if not os.path.isdir(modules_dir):
                log_and_print(
                    f"{FAILURE} /usr/lib/modules directory not found! Cannot clean old kernels.",
                    "error",
                )
                return
            ...
            if kernels_to_remove:
                log_and_print(
                    f"{INFO} Kernels/folders to remove: {kernels_to_remove}", "info"
                )
                confirm = prompt_with_timeout(
...
def update_font_cache():
    ...
    with spinning_spinner():
        try:
            result = subprocess.run(
                ["sudo", "fc-cache", "-fv"], check=True, capture_output=True, text=True
            )
            log_and_print(
                ...
                if os.path.isdir(item):
                    ...
                    if pkg_dir not in installed_packages:
                        ...
                        if confirm == "y":
                            ...
                            log_and_print(
                                f"{INFO} Deleted source directory: {pkg_dir}", "info"
                            )
    ...
    with spinning_spinner():
        ...
        try:
            pacnew_files = subprocess.check_output(
                ["sudo", "find", "/etc", "-type", "f", "-name", "*.pacnew"], text=True
            ).splitlines()
            pacsave_files = subprocess.check_output(
                ["sudo", "find", "/etc", "-type", "f", "-name", "*.pacsave"], text=True
            ).splitlines()
                ...
                for file in pacnew_files:
                    ...
                    if action == "m":
                        ...
                        try:
                            ...
                            os.remove(file)
                            actions_summary.append(
                                f"Replaced with .pacnew file: {file}"
                            )
                        except Exception as ex:
                            log_and_print(
                ...
                for file in pacsave_files:
                    ...
                    if action == "r":
                        ...
                        try:
                            ...
                            os.remove(file)
                            actions_summary.append(
                                f"Restored from .pacsave file: {file}"
                            )
                        except Exception as ex:
                            log_and_print(
    ...
    with spinning_spinner():
        try:
            ...
            zram_device = subprocess.check_output(
                ["sudo", "zramctl", "--find", "--size", str(mem_total)], text=True
            ).strip()
...
def adjust_swappiness(value=10, force=False):
    ...
    try:
        ...
        log_and_print(
            f"{FAILURE} Error: Failed to adjust swappiness: {e.stderr.strip()}", "error"
        )
...
def security_audit():
    ...
    try:
        ...
        if tun0_active:
            log_and_print(
                f"{INFO} ExpressVPN tun0 is active. Checking for listening services on main interface...",
                "info",
            )
            interface = "enp2s0"
            nmap_cmd = [
