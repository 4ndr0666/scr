...
console = Console()
logging.basicConfig(
    filename="file_management_v2_0.log",
    level=logging.INFO,
...
def handle_duplicates(file_path, target_directory):
    ...
    file_hash = generate_file_hash(file_path)[
        :8
    ]  # using first 8 characters for brevity
    counter = 1
    while new_path.exists():
        new_path = (
            target_directory
            / f"{original_path.stem}_{file_hash}_{counter}{original_path.suffix}"
        )
        counter += 1
    return new_path
...
def flatten_directory(directory):
    ...
    if not directory_path.is_dir():
        console.print(
            f"[bold red]The specified directory does not exist: {directory}[/bold red]"
        )
        return
...
def remove_empty_subdirectories(directory):
    ...
    empty_dirs = [
        d for d in directory_path.rglob("*") if d.is_dir() and not any(d.iterdir())
    ]
    for d in empty_dirs:
        d.rmdir()
...
def main_menu():
    ...
    if choice == "1":
        ...
        method = Prompt.ask(
            "[bold cyan]Choose organization method (alphabet/date/type)[/bold cyan]",
            choices=["alphabet", "date", "type"],
        )
        ...
        directory = Prompt.ask(
            "[bold cyan]Enter the directory path for duplicate checking[/bold cyan]"
        )
        find_duplicates(directory)
    elif choice == "4":
        directory = Prompt.ask(
            "[bold cyan]Enter the directory path to flatten[/bold cyan]"
        )
        flatten_directory(directory)
    elif choice == "5":
        directory = Prompt.ask(
            "[bold cyan]Enter the directory path to remove empty subdirectories[/bold cyan]"
        )
