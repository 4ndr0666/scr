...
def process_regular_archive(archive_path, conn, tqdm_module):
    ...
    try:
        ...
        cursor.execute(
            "SELECT original_path FROM files WHERE source_archive = ? AND final_path IS NOT NULL",
            (archive_name,),
        )
        completed_files = {row[0] for row in cursor.fetchall()}
        if completed_files:
            print(
                f"    ✅ Found {len(completed_files)} completed files. Will skip unpacking them."
            )
        else:
...
def deduplicate_files(
    source_path, primary_storage_path, trash_path, conn, source_archive_name
):
    ...
    if os.path.exists(takeout_source):
        ...
        for root, _, files in os.walk(takeout_source):
            for filename in files:
                ...
                cursor.execute(
                    "UPDATE files SET final_path = ? WHERE original_path = ? AND source_archive = ?",
        ...
        if files_moved > 0:
            conn.commit()
    print(
        f"    ✅ Deduplication and move complete. Processed {files_moved} unique files."
    )
...
def organize_photos(source_path, final_dir, conn, tqdm_module, source_archive_name):
    ...
    for json_path in tqdm_module(all_json_files, desc="Organizing photos"):
        ...
        try:
            ...
            cursor.execute(
                "UPDATE files SET final_path = ?, timestamp = ? WHERE original_path = ? AND source_archive = ?",
