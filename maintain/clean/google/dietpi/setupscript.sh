#!/bin/bash
#
# DietPi First-Boot Custom Script for Takeout Processor Appliance (v2.5)
# This is the definitive installer for the final, portable Python script.

set -euo pipefail

# --- Configuration ---
INSTALL_DIR="/opt/google_takeout_organizer" # Corrected directory name
...
main() {
    ...
    apt-get update -y
    apt-get install -y python3 jdupes sqlite3 curl python3-tqdm util-linux
    ...
    # This sed command is now the definitive fix for making the script portable
    sed -i '/from google.colab import drive/d' "$PROCESSOR_SCRIPT_PATH"
    sed -i '/drive.mount/d' "$PROCESSOR_SCRIPT_PATH"
