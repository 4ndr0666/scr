...
# It is fully non-interactive and includes robust verification checks.

set -euo pipefail

# --- Configuration ---
INSTALL_DIR="/opt/takeout_processor"
# This URL must point to the final, v4.1 filesystem-based Python script.
...
main() {
    ...
    # findmnt is part of the util-linux package, which is essential and should already be present.
    apt-get install -y \
        python3 \
        jdupes \
        sqlite3 \
    ...
    # --- 3. Create & VERIFY Data Directories ---
    ...
    # --- CRITICAL: Verify that DATA_DIR is a mount point ---
...
EOF
    ...
    _log_info "The system is now configured to automatically process your archives."
    _log_info "1. Place your Takeout .tgz files in: ${base_project_dir}/00-ALL-ARCHIVES/"
    _log_info "2. Reboot the system to begin the automated processing."
