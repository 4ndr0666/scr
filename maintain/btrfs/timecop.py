...
def schedule_task(cron_expression, command):
    ...
    execute_command(
        f'(crontab -l ; echo "{cron_job}") | sort - | uniq - | crontab -', shell=True
    )
...
def run_all_tasks():
    ...
    min_age_days = int(
        input("Enter the minimum age of snapshots to delete (in days): ")
    )
...
def show_help():
    ...
    print(
        f"{GREEN}1{NC}) Create Snapshot        - Create a snapshot of a specified subvolume"
    )
    print(f"{GREEN}2{NC}) Delete Snapshot        - Delete a specified snapshot")
    print(f"{GREEN}3{NC}) List Snapshots         - List all snapshots")
    print(
        f"{GREEN}4{NC}) Cleanup Snapshots      - Cleanup old snapshots based on specified age"
    )
    print(
        f"{GREEN}5{NC}) Update fstab UUID      - Update the UUID in /etc/fstab for a specified device"
    )
    print(
        f"{GREEN}6{NC}) Update GRUB UUID       - Update the UUID in GRUB configuration for a specified device"
    )
    print(
        f"{GREEN}7{NC}) Freeze Operations      - Freeze Btrfs operations on a specified mount point"
    )
    print(
        f"{GREEN}8{NC}) Thaw Operations        - Thaw Btrfs operations on a specified mount point"
    )
    print(
        f"{GREEN}9{NC}) Schedule Task          - Schedule an automated task using cron"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Manage Btrfs Snapshots with Extended Features"
    )
    ...
    while True:
        ...
        if command == "1":
            ...
            min_age_days = int(
                input("Enter minimum age of snapshots to delete (in days): ")
            )
            cleanup_snapshots(min_age_days)
        elif command == "5":
            ...
            task = input(
                "Enter task to automate (create-snapshot/cleanup-snapshots/update-fstab/update-grub): "
            )
            if task == "create-snapshot":
                ...
                label = input("Enter snapshot label (optional): ")
                command = (
                    f"python3 manage_snapshots.py create-snapshot {subvolume} {snapshot_type} --label {label}"
                    if label
                    else f"python3 manage_snapshots.py create-snapshot {subvolume} {snapshot_type}"
                )
                ...
                command = (
                    f"python3 manage_snapshots.py cleanup-snapshots {min_age_days}"
                )
