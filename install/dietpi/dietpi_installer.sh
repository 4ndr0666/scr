...
CFG_SWAPFILE_SIZE=0
CFG_SWAPFILE_LOCATION=/var/swap
CFG_NET_ETHERNET_ENABLED=1     # 1=enable Ethernet (WiFi takes priority if both enabled)
CFG_DHCP_TO_STATIC=0           # 1=convert DHCP lease to static on first boot
CFG_LOGGING_INDEX=-1           # -1=RAMlog hourly clear  -2=RAMlog+save  -3=Rsyslog
CFG_DESKTOP=none               # none | lxde | xfce | mate | lxqt | gnustep
CFG_SERIAL_CONSOLE=0           # 0=disable serial console (recommended for headless)
CFG_SSH_DISABLE_PASS_LOGINS=0  # 0=allow  root=root only  1=all users
...
${B}System (additional):${Z}
  --swapfile-location P   AUTO_SETUP_SWAPFILE_LOCATION   (default: /var/swap)
  --ethernet N            AUTO_SETUP_NET_ETHERNET_ENABLED 1=on 0=off (default: 1)
  --dhcp-to-static        Convert first DHCP lease to static on first boot
  --logging-index N       AUTO_SETUP_LOGGING_INDEX -1=RAMlog -2=RAMlog+save -3=Rsyslog
  --desktop NAME          none | lxde | xfce | mate | lxqt | gnustep (default: none)
  --serial-console N      CONFIG_SERIAL_CONSOLE_ENABLE 0=off 1=on (default: 0)
...
parse_args() {
	while [[ $# -gt 0 ]]; do
		...
		--kernel-config)
			...
			;;
		--swapfile-location)
			CFG_SWAPFILE_LOCATION="$2"
			shift 2
			;;
		--ethernet)
			CFG_NET_ETHERNET_ENABLED="$2"
			shift 2
			;;
		--dhcp-to-static)
			CFG_DHCP_TO_STATIC=1
			shift
			;;
		--logging-index)
			CFG_LOGGING_INDEX="$2"
...
# ── Validate ──────────────────────────────────────────────────────────────────
validate_config() {
	# Warn when AUTO_SETUP_AUTOMATED=1 and password is still the insecure default.
	if [[ "${CFG_PASSWORD}" == "dietpi" ]]; then
		warn "AUTO_SETUP_GLOBAL_PASSWORD is still the default (dietpi). Change it with --password."
	fi
	if [[ "${DRY_RUN}" -eq 0 ]]; then
...
generate_dietpi_txt() {
	...
	sed -i "s|^#*AUTO_SETUP_SWAPFILE_LOCATION=.*|AUTO_SETUP_SWAPFILE_LOCATION=${CFG_SWAPFILE_LOCATION}|" "${txt}"
	sed -i "s|^#*AUTO_SETUP_NET_ETHERNET_ENABLED=.*|AUTO_SETUP_NET_ETHERNET_ENABLED=${CFG_NET_ETHERNET_ENABLED}|" "${txt}"
	sed -i "s|^#*AUTO_SETUP_DHCP_TO_STATIC=.*|AUTO_SETUP_DHCP_TO_STATIC=${CFG_DHCP_TO_STATIC}|" "${txt}"
	sed -i "s|^#*AUTO_SETUP_LOGGING_INDEX=.*|AUTO_SETUP_LOGGING_INDEX=${CFG_LOGGING_INDEX}|" "${txt}"
	sed -i "s|^#*AUTO_SETUP_DESKTOP=.*|AUTO_SETUP_DESKTOP=${CFG_DESKTOP}|" "${txt}"
	sed -i "s|^#*CONFIG_SERIAL_CONSOLE_ENABLE=.*|CONFIG_SERIAL_CONSOLE_ENABLE=${CFG_SERIAL_CONSOLE}|" "${txt}"
	sed -i "s|^#*SOFTWARE_DISABLE_SSH_PASSWORD_LOGINS=.*|SOFTWARE_DISABLE_SSH_PASSWORD_LOGINS=${CFG_SSH_DISABLE_PASS_LOGINS}|" "${txt}"
...
generate_wifi_txt() {
	...
	# Fetch the canonical template so all 5 entries (0-4) and every field
	# (SSID KEY KEYMGR PROTO PAIRWISE AUTH_ALG EAP IDENTITY PASSWORD
	...
	local safe_ssid="${CFG_WIFI_SSID//\'/\'\'\'}"
	local safe_pass="${CFG_WIFI_PASS//\'/\'\'\'}"

	sed -i "s|^aWIFI_SSID\[0\]=.*|aWIFI_SSID[0]='${safe_ssid}'|"  "${txt}"
	sed -i "s|^aWIFI_KEY\[0\]=.*|aWIFI_KEY[0]='${safe_pass}'|"     "${txt}"
	sed -i "s|^aWIFI_KEYMGR\[0\]=.*|aWIFI_KEYMGR[0]='WPA-PSK'|"   "${txt}"
	# Slots 1-4 and all WPA-EAP fields remain at canonical blank defaults.

	log "dietpi-wifi.txt ready â ${txt}"
}
...
# Triggered by: AUTO_SETUP_CUSTOM_SCRIPT_EXEC=0 (local) or a URL (remote fetch).
# The script must exist at /boot/Automation_Custom_Script.sh on the FAT partition.
...
# ── Rice user ────────────────────────────────────────────────────────────────
# Escape single-quotes before embedding in the generated script.
RICE_USER="${RICE_USER//\'/\'\'\'}"
