...
#                    configures DietPi tools, hardens sudoers, etc.
#
#  Usage:
#    sudo ./dietpi_rpi4_install.sh [OPTIONS]
#    sudo ./dietpi_rpi4_install.sh --noninteractive --device /dev/sdb \
#         --hostname mypi --password s3cr3t --timezone America/New_York
...
# ── dietpi.txt knobs ──────────────────────────────────────────────────────────
CFG_HOSTNAME="DietPi-RPi4"
CFG_PASSWORD="dietpi"
CFG_LOCALE="en_US.UTF-8"
CFG_TIMEZONE="America/New_York"
CFG_KEYBOARD="us"
CFG_SSH_INDEX="-2"          # -2=OpenSSH  0=Dropbear  -1=none
CFG_WIFI_ENABLED=0
CFG_WIFI_SSID=""
CFG_WIFI_PASS=""
CFG_WIFI_COUNTRY="US"
...
log()  { printf "${G}[*]${Z} %s\n"  "$*"; }
warn() { printf "${Y}[!]${Z} %s\n"  "$*" >&2; }
fatal(){ printf "${R}[X]${Z} %s\n"  "$*" >&2; exit 1; }
info() { printf "${C}[i]${Z} %s\n"  "$*"; }
ask()  { printf "${C}[?]${Z} %s"    "$*"; }

# ── run — bounded subprocess ──────────────────────────────────────────────────
run() { local s="$1"; shift; timeout --foreground "$s" "$@"; }
...
_cleanup() {
    ...
    _cleanup_done=1
    set +e
    [[ -n "${_MOUNT_POINT}" ]] && { sync; umount "${_MOUNT_POINT}" 2>/dev/null; rmdir "${_MOUNT_POINT}" 2>/dev/null; }
    exit "$rc"
}
...
${B}progs.csv tag reference:${Z}
  D,93,Pi-hole             dietpi-software ID 93
  A,git,Version control    apt install git
  G,https://…/st.git,Suckless terminal   git clone + make install
  P,speedtest-cli,CLI speedtest           pip3 install

${B}Examples:${Z}
  # Interactive TUI wizard:
  sudo ${SCRIPT_NAME}

  # Fully scripted, WiFi, Pi-hole + Nextcloud, dotfiles:
  sudo ${SCRIPT_NAME} --device /dev/sdb --hostname mypi --password s3cr3t \\
      --timezone America/New_York --wifi-ssid "Home" --wifi-pass "s3cr3t" \\
      --software-ids "93 114" --dotfiles-repo https://github.com/you/dots.git

  # Dry-run only — inspect generated files in ./dietpi_build/:
  ${SCRIPT_NAME} --dry-run --hostname testpi
USAGE
}
...
parse_args() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --hostname)         CFG_HOSTNAME="$2";        shift 2 ;;
            --password)         CFG_PASSWORD="$2";        shift 2 ;;
            --locale)           CFG_LOCALE="$2";          shift 2 ;;
            --timezone)         CFG_TIMEZONE="$2";        shift 2 ;;
            --keyboard)         CFG_KEYBOARD="$2";        shift 2 ;;
            --ssh-server)       CFG_SSH_INDEX="$2";       shift 2 ;;
            --survey)           CFG_SURVEY_OPT="$2";      shift 2 ;;
            --wifi-ssid)        CFG_WIFI_SSID="$2"; CFG_WIFI_ENABLED=1; shift 2 ;;
            --wifi-pass)        CFG_WIFI_PASS="$2";       shift 2 ;;
            --wifi-country)     CFG_WIFI_COUNTRY="$2";    shift 2 ;;
            --static-ip)        CFG_STATIC_ADDR="$2"; CFG_STATIC_IP=1; shift 2 ;;
            --static-mask)      CFG_STATIC_MASK="$2";     shift 2 ;;
            --static-gw)        CFG_STATIC_GW="$2";       shift 2 ;;
            --static-dns)       CFG_STATIC_DNS="$2";      shift 2 ;;
            --software-ids)     CFG_SOFTWARE_IDS="$2";    shift 2 ;;
            --apt-packages)     CFG_APT_PACKAGES="$2";    shift 2 ;;
            --autostart)        CFG_AUTOSTART_INDEX="$2"; shift 2 ;;
            --autostart-user)   CFG_AUTOSTART_USER="$2";  shift 2 ;;
            --rice-user)        RICE_USER="$2";           shift 2 ;;
            --rice-user-pass)   RICE_USER_PASS="$2";      shift 2 ;;
            --dotfiles-repo)    DOTFILES_REPO="$2";       shift 2 ;;
            --dotfiles-branch)  DOTFILES_BRANCH="$2";     shift 2 ;;
            --progs-csv)        PROGS_CSV_URL="$2";       shift 2 ;;
...
# ── whiptail TUI wizard ───────────────────────────────────────────────────────
# Mirrors 4ndr0666.sh's UX pattern: each section is a whiptail dialog.
# Falls back to plain read() if whiptail is absent.
_wt_available() { command -v whiptail >/dev/null 2>&1; }

_wt_input() {
    # _wt_input VARNAME "Prompt" "Default"
    local var="$1" prompt="$2" default="$3"
    local val
    if _wt_available; then
        val=$(whiptail --inputbox "${prompt}" 10 60 "${default}" 3>&1 1>&2 2>&3) || return 1
    else
        ask "${prompt} [${default}]: "; read -r val; val="${val:-${default}}"
    fi
    printf -v "${var}" '%s' "${val}"
}
...
_wt_pass() {
    ...
    while true; do
        if _wt_available; then
            ...
            ask "${prompt}: "; read -rs p1; echo
            ask "Confirm: ";   read -rs p2; echo
        fi
        [[ "${p1}" == "${p2}" ]] && break
        _wt_available \
...
_wt_yesno() {
    ...
    if _wt_available; then
        ...
        ask "${prompt} [y/N]: "; local a; read -r a; [[ "${a,,}" == "y" ]]
    fi
}
...
run_wizard() {
    ...
    _wt_input CFG_TIMEZONE "Timezone (tz database, e.g. America/New_York):" "${CFG_TIMEZONE}"
    ...
    if _wt_available; then
        CFG_SSH_INDEX=$(whiptail --title "SSH Server" \
            ...
            "-2" "OpenSSH (recommended)" \
            "0"  "Dropbear (lightweight)" \
            "-1" "None" \
            3>&1 1>&2 2>&3) || CFG_SSH_INDEX="-2"
    fi
    ...
    if _wt_available; then
        ...
        # whiptail returns quoted strings; strip quotes and commas
...
fetch_image() {
    ...
    if [[ ! -f "${img}" ]]; then
        [[ -f "${xz}" ]] || { log "Downloading DietPi image..."; _dl "${DIETPI_IMAGE_URL}" "${xz}"; }
        log "Verifying checksum..."; _dl "${DIETPI_SHA256_URL}" "${sha}"
        local exp act
        exp="$(awk '{print $1}' "${sha}")"
...
generate_dietpi_txt() {
    ...
    for id in ${CFG_SOFTWARE_IDS}; do
        sw_lines+="AUTO_SETUP_INSTALL_SOFTWARE_ID=${id}"$'\n'
    done
    local apt_line=""
    [[ -n "${CFG_APT_PACKAGES}" ]] && apt_line="AUTO_SETUP_APT_INSTALLS=${CFG_APT_PACKAGES}"

    cat > "${dir}/dietpi.txt" <<DIETPI
# ============================================================================
# DietPi automated first-boot configuration
# Generated by ${SCRIPT_NAME} v${SCRIPT_VERSION} — $(date -u +%Y-%m-%dT%H:%M:%SZ)
# ============================================================================
...
##### Locale / System #####
AUTO_SETUP_LOCALE=${CFG_LOCALE}
AUTO_SETUP_KEYBOARD_LAYOUT=${CFG_KEYBOARD}
AUTO_SETUP_TIMEZONE=${CFG_TIMEZONE}
AUTO_SETUP_NET_HOSTNAME=${CFG_HOSTNAME}
AUTO_SETUP_GLOBAL_PASSWORD=${CFG_PASSWORD}

##### SSH #####
# -2=OpenSSH  0=Dropbear  -1=None
AUTO_SETUP_SSH_SERVER_INDEX=${CFG_SSH_INDEX}

##### Network — WiFi #####
AUTO_SETUP_NET_WIFI_ENABLED=${CFG_WIFI_ENABLED}
AUTO_SETUP_NET_WIFI_COUNTRY_CODE=${CFG_WIFI_COUNTRY}

##### Network — Static IP (0=DHCP 1=Static) #####
AUTO_SETUP_NET_USESTATIC=${CFG_STATIC_IP}
AUTO_SETUP_NET_STATIC_IP=${CFG_STATIC_ADDR}
AUTO_SETUP_NET_STATIC_MASK=${CFG_STATIC_MASK}
AUTO_SETUP_NET_STATIC_GATEWAY=${CFG_STATIC_GW}
AUTO_SETUP_NET_STATIC_DNS=${CFG_STATIC_DNS}

##### Updates #####
# 0=off  1=check  2=check+install
CONFIG_CHECK_DIETPI_UPDATES=${CFG_AUTO_UPDATE}
CONFIG_CHECK_APT_UPDATES=${CFG_APT_UPDATE}

##### Autostart (0=console 16=LXDE 7=Kodi) #####
AUTO_SETUP_AUTOSTART_TARGET_INDEX=${CFG_AUTOSTART_INDEX}
AUTO_SETUP_AUTOSTART_LOGIN_USER=${CFG_AUTOSTART_USER}

##### DietPi Survey (0=opt-out 1=opt-in) #####
SURVEY_OPTED_IN=${CFG_SURVEY_OPT}
...
generate_wifi_txt() {
    ...
    cat > "${dir}/dietpi-wifi.txt" <<WIFI
# DietPi WiFi — generated by ${SCRIPT_NAME}
aWIFI_SSID[0]='${CFG_WIFI_SSID}'
aWIFI_KEY[0]='${CFG_WIFI_PASS}'
# aWIFI_SSID[1]=''
# aWIFI_KEY[1]=''
WIFI
    log "dietpi-wifi.txt → ${dir}/dietpi-wifi.txt"
}
...
generate_post_script() {
    ...
    # Embed the progs.csv inline if a local path was provided; otherwise
    # the script will download it at runtime from PROGS_CSV_URL.
...
# ── DietPi shell function wrappers ────────────────────────────────────────────
# These are the non-interactive apt wrappers documented in DietPi misc tools.
# G_AGI  = apt-get install (error-handled, retryable)
# G_AGUP = apt-get update
# G_AGUG = apt-get upgrade
# They are available after DietPi first-boot; source DietPi's func lib.
...
setup_user() {
    ...
    # Passwordless sudo for wheel-equivalent ops (mirrors 4ndr0666.sh pattern)
...
# ── Home directory for rice target ───────────────────────────────────────────
rice_home() {
...
# Tags:
#   D  dietpi-software ID  (installed via dietpi-software install <id>)
#   A  APT package name
#   G  git URL             (cloned + make + make install)
#   P  pip3 package name
...
run_progs_loop() {
    ...
    while IFS=, read -r tag name desc; do
        n=\$((n+1))
        # Strip surrounding quotes from desc
...
deploy_dotfiles() {
    ...
    # rsync everything except .git metadata (mirrors 4ndr0666.sh putgitrepo)
...
harden_system() {
    ...
    # Silence PC speaker (mirrors 4ndr0666.sh rmmod pcspkr)
    rmmod pcspkr 2>/dev/null || true
    echo "blacklist pcspkr" > /etc/modprobe.d/nobeep.conf

    # dmesg readable by normal users
    mkdir -p /etc/sysctl.d
    echo "kernel.dmesg_restrict = 0" > /etc/sysctl.d/dmesg.conf
    sysctl -p /etc/sysctl.d/dmesg.conf >/dev/null 2>&1 || true

    # Useful shell aliases in /etc/profile.d
    cat > /etc/profile.d/dietpi_rice.sh <<'PROFILE'
# DietPi rice — convenience aliases
...
# ── DietPi misc-tools configuration ──────────────────────────────────────────
configure_dietpi_tools() {
    log "Configuring DietPi misc tools..."

    # Survey: value was baked into dietpi.txt; confirm via CLI just in case
    if command -v dietpi-survey >/dev/null 2>&1; then
        dietpi-survey 1 >/dev/null 2>&1 || true
    fi

    # Servarr-to-RAM: enable only if any Servarr apps were installed
    ...
    # Run a quick benchmark silently and save results (non-blocking)
