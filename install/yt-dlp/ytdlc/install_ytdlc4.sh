...
if (( ${#PKGS_TO_INSTALL[@]} > 0 )); then
    ...
    ans=${ans:-Y}
    if [[ "$ans" =~ ^[Yy]$ ]]; then
        sudo pacman -S --needed --noconfirm "${PKGS_TO_INSTALL[@]}" > /dev/null 2>&1
    else
...
# Remove immutability if the file exists
if [[ -f "$YTDL_FILE" ]]; then
    sudo chattr -i "$YTDL_FILE" > /dev/null 2>&1 || true
fi
...
chmod 644 "$YTDL_FILE" > /dev/null 2>&1 || true
...
if ! grep -q 'source $HOME/.config/zsh/ytdl.zsh' "$SHELL_RC" > /dev/null 2>&1; then
...
ans=${ans:-Y}
if [[ "$ans" =~ ^[Yy]$ ]]; then
    declare YTDL_HANDLER
    YTDL_HANDLER="$BIN_DIR/ytdl-handler.sh"
    if [[ -f "$YTDL_HANDLER" ]]; then
        sudo chattr -i "$YTDL_HANDLER" > /dev/null 2>&1 || true
    fi
    sudo tee "$YTDL_HANDLER" > /dev/null 2>&1 << 'HND_EOF'
#!/bin/sh
# Author: 4ndr0666
...
DESK_EOF
    ...
    xdg-mime default ytdl.desktop x-scheme-handler/ytdl > /dev/null 2>&1
