...
class images(scrapy.Spider):
    ...
    start_urls = [
        "https://cdn.bunnylust.com/app/uploads/2023/06/lana-lane-black-lingerie-cosmid-11.jpg"
    ]
    ...
    def parse(self, response):
        ...
        for link in dir_links:
            ...
            yield scrapy.Request(
                full_link, callback=self.parse, errback=self.handle_error
            )

    def handle_error(self, failure):
        self.logger.error(
            f"Request failed: {failure.request.url} with error: {failure.value}"
        )
