...
# Removed execute_command function that called `mycmd` (unused)
# Previously here:
# execute_command() { ... } # Removed due to `mycmd` issues
...
trap 'cleanup' EXIT
cleanup() {
    # Cleanup temporary media info file
    rm -f /tmp/stream_media_info.json 2>/dev/null || true
}

# --- // Ffprobe & Extract Media Info:
extract_media_info() {
    local stream_url="$1"
    ...
    # Write media data to a known file
    ...
    # Convert fraction framerate if needed
    if [[ "$framerate" == *"/"* ]]; then
...
adjust_settings_based_on_media() {
    ...
    if extract_media_info "$stream_url"; then
        # Now we have /tmp/stream_media_info.json
        local resolution
        resolution=$(jq -r '.streams[] | select(.codec_type=="video") | .height' /tmp/stream_media_info.json | head -n1)
        [ -z "$resolution" ] && resolution=0

        if [[ "$resolution" -ne 0 && "$resolution" -lt 720 ]]; then
...
run_streamlink() {
    ...
    while [ $retries -lt $MAX_RETRIES ]; do
        ...
        # Added CACHE_OPTION
        streamlink "$url" "$quality" --output "$final_output_file" $RETRY_STREAMS $HLS_OPTIONS $PROXY_OPTION $CACHE_OPTION > "$final_log_file" 2>&1 &
        local pid=$!
        ...
        if [ $exit_code -eq 0 ]; then
            ...
            display_message error "Streamlink execution failed (exit code: $exit_code). Retrying in $RETRY_DELAY seconds..."
            sleep $RETRY_DELAY
        fi
        retries=$((retries + 1))
    done
