...
check_programs() {
    required_programs=("arch-chroot" "pacman" "mount" "sed" "blkid")
...
mount_filesystems() {
    ...
    read -p "Enter the mount point for the root partition (e.g., /mnt): " mount_point
    ...
    for subvol in @home @root @srv @cache @log @tmp; do
        mkdir -p "$mount_point/$(echo $subvol | sed 's/@//')"
        mount -o subvol=/$subvol "$root_partition" "$mount_point/$(echo $subvol | sed 's/@//')"
        if [ $? -ne 0 ]; then
...
# Helper function: bg-rootshell
bg_rootshell() {
    exec bash
}

# Helper function: rootshell
rootshell() {
    exec bash "$@"
}
