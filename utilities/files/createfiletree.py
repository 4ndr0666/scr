...
def create_project_structure(base_path, project_name):
    ...
    if os.path.exists(project_directory):
        print(f"{RED}❌ Error: The project directory '{project_directory}' already exists.{c0}")
        return
    ...
    for filename, content in templates.items():
        ...
        if not os.path.exists(file_path):
            with open(file_path, 'w') as file:
                file.write(content)
            print(f"{GRE}📝 Template file '{filename}' created in '{project_directory}'{c0}")
    ...
    while True:
        ...
        if command in build_options:
            ...
            if not os.path.isdir(base_path):
                print(f"{CYAN}⚠️ The path '{base_path}' does not exist or is invalid. Please provide a valid directory path.{c0}")
                continue
