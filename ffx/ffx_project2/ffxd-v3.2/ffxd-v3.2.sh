#!/bin/sh
# ffxd-v3.3.sh — Bulk video toolkit: merge/composite/process/batch with XDG, grid, and advanced mode.
# Requires: POSIX shell, ffmpeg, ffprobe, find, mktemp
...
TMPFILES=""
trap 'printf "%s" "$TMPFILES" | while read -r f; do [ -f "$f" ] && rm -f "$f"; done' EXIT
...
wizard_prompt() {
  printf '%s\n' "--- Advanced Wizard (cfg at $CFG_FILE) ---"
...
normalize_inputs() {
  ...
  for inpf; do
    ...
    if ffprobe -v error -select_streams a -show_entries stream=index -of csv=p=0 "$inpf" >/dev/null 2>&1; then
      set -- -c:a aac -b:a 128k
    else
      set -- -an
    fi
    ffmpeg -y -i "$inpf" -vf "$sf" -r "$ADV_FPS" \
      -c:v libx264 -qp 0 -pix_fmt yuv420p -preset veryfast "$@" "$tmp" || {
        echo "ffmpeg failed on: $inpf" >&2; exit 1;
    }
...
'
  ...
  if [ "$COMPOSITE" -eq 1 ]; then
    ...
    cmd="$cmd -filter_complex '$filt' -map '[v]' -c:v libx264 -qp 0 -pix_fmt yuv420p -preset medium \"$OUTPUT\""
  else
    filt="[0:v][0:a]"
    i=1
    while [ "$i" -lt "$#" ]; do filt="$filt[${i}:v][${i}:a]"; i=$((i+1)); done
    filt="$filt concat=n=$#:v=1:a=1[v][a]"
    cmd="ffmpeg -y"; for f; do cmd="$cmd -i \"$f\""; done
    cmd="$cmd -filter_complex '$filt' -map '[v]' -map '[a]' -c:v libx264 -qp 0 -pix_fmt yuv420p -preset medium -c:a aac -b:a 128k \"$OUTPUT\""
  fi
  eval "$cmd"
...
cmd_process() {
  ...
  in=$1 out=$2
  ffmpeg -y -i "$in" -c copy -movflags +faststart "$out" >/dev/null 2>&1 && {
    printf '✅ Repaired: %s\n' "$out"; return;
  }
  ffmpeg -y -fflags +genpts -i "$in" -c:v copy -c:a aac -movflags +faststart "$out" >/dev/null 2>&1 && {
    printf '✅ Repaired: %s\n' "$out"; return;
  }
...
'
  ...
  if [ "$filt" = null ]; then
    ...
    cmd="ffmpeg -y"; for f; do cmd="$cmd -i \"$f\""; done
    cmd="$cmd -filter_complex '$filt' -map '[v]' -c:v libx264 -qp 0 -pix_fmt yuv420p -preset medium \"$out\""
    eval "$cmd"
  fi
