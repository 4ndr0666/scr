...
def prompt_with_timeout(prompt: str, timeout: int = 30, default: str = 'Q') -> str:
    ...
    Args:
        ...
        timeout (int): The number of seconds to wait before returning the default.
        default (str): The default value to return if the user does not provide input.
        
    Returns:
        str: The user's input or the default value if input times out.
    """
    ...
    with spinning_spinner():
        try:
            ...
            if kernels_to_remove:
                ...
                confirm = prompt_with_timeout("Do you want to remove these kernels? [y/N]: ", timeout=10, default='n').lower()
                if confirm == 'y':
            ...
            try:
                for root, dirs, files in os.walk(cache_dir):
                    for file in files:
                        ...
                        if re.search(r'\.aria2$|\.part$|\.crdownload$|\.unrecognized$', file):
                            problematic_files.append(file_path)

                for file_path in problematic_files:
                    try:
                        os.remove(file_path)
                        log_and_print(f"{SUCCESS} Removed problematic file: {file_path}", 'info')
                    except FileNotFoundError:
                        log_and_print(f"{INFO} File not found, skipping: {file_path}", 'info')
                    except OSError as os_error:
                        log_and_print(f"{FAILURE} Failed to remove file {file_path}: {os_error}", 'error')

            except Exception as scan_error:
                log_and_print(f"{FAILURE} Error scanning for problematic files: {scan_error}", 'error')
...
def handle_pacnew_pacsave(log_file):
    ...
    with spinning_spinner():
        try:
            ...
            if pacnew_files:
                log_and_print(f"{INFO} Found .pacnew files: {pacnew_files}", 'info')
                for file in pacnew_files:
                    os.remove(file)
                    actions_summary.append(f"Removed .pacnew file: {file}")

            if pacsave_files:
                log_and_print(f"{INFO} Found .pacsave files. Reviewing each:", 'info')
                for file in pacsave_files:
                    action = prompt_with_timeout(
                        f"Do you want to remove, skip, or overwrite {file}? (remove/skip/overwrite): ",
                    ...
                    if action == "remove":
                        ...
                        log_and_print(f"{FAILURE} Invalid action '{action}' for file: {file}. Skipping...", 'error')
                        actions_summary.append(f"Skipped .pacsave file: {file} due to invalid input")
...
def main():
    ...
    while True:
        ...
        # Use the prompt_with_timeout function to handle user input with timeout and default value
        command = prompt_with_timeout("Enter your choice: ", timeout=30, default='Q').strip().upper()
