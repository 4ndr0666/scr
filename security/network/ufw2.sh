...
## Auto-escalate privileges if not root
if [[ "${EUID}" -ne 0 ]]; then
    log "💀WARNING💀 - you are not root. Attempting to escalate..."
    exec sudo "$0" "$@"
    exit $?
fi
...
net.ipv4.tcp_congestion_control=bbr"
    ...
    if [[ ! -f "$SYSCTL_IPV4_FILE" ]] || ! grep -qF "net.core.rmem_max=16777216" "$SYSCTL_IPV4_FILE"; then
        log "Creating/updating $SYSCTL_IPV4_FILE..."
