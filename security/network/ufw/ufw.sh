...
parse_args() {
	...
	while [[ $# -gt 0 ]]; do
		case "$1" in
		--vpn) VPN_FLAG=1; log INFO "Option: --vpn enabled" ;;
		--disconnect) DISCONNECT_FLAG=1; log INFO "Option: --disconnect enabled" ;;
		--jdownloader) JD_FLAG=1; log INFO "Option: --jdownloader enabled" ;;
		--backup) BACKUP_FLAG=1; log INFO "Option: --backup enabled" ;;
		--silent) SILENT=1; log INFO "Option: --silent enabled" ;;
		--dry-run) DRY_RUN=1; log INFO "Option: --dry-run enabled" ;;
		--status) STATUS_FLAG=1; log INFO "Option: --status enabled" ;;
		--swappiness)
			if [[ -n "${2:-}" && "${2}" =~ ^[0-9]+$ ]]; then
				SWAPPINESS_VAL="${2}"
...
main() {
	...
	if ((VPN_FLAG)); then
		...
		expressvpn_connect || {
			log ERROR "ExpressVPN connection failed. The kill switch will block all outbound traffic."
		}
