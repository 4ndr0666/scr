...
# (matches the selection UX already used by `vpn_select` in the zsh
# functions, so both tools agree on "one profile, one truth" instead of
# silently picking different tunnels).
select_wg_profile() {
	...
	if command -v fzf >/dev/null 2>&1 && [[ "$SILENT" -eq 0 && "$DRY_RUN" -eq 0 ]]; then
...
megavpn_connect() {
	...
	# Tear down any other active WireGuard tunnels first to prevent routing
	# conflicts, mirroring vpn_select's pre-switch teardown behavior.
	local active_conns
