...
# Ensure log directory and file exist before any logging occurs
mkdir -p "$LOG_DIR"
touch "$LOG_FILE"
chmod 600 "$LOG_FILE"
...
apply_dns_rules() {
	...
	for rule in "${dns_rules[@]}"; do
		if validate_ufw_rule "$rule"; then
			read -r -a parsed_rule <<<"$rule"
			run_cmd_dry ufw "${parsed_rule[@]}" ||
				log "WARN" "Failed DNS rule: $rule"
		fi
	done
	return 0
}
...
usage() {
	...
	printf '%s\n' "Usage: $SCRIPT_NAME [options]" "" "Options:" \
...
configure_ufw() {
	...
	for rule_spec in "${ALL_RULES[@]}"; do
		if validate_ufw_rule "$rule_spec"; then
			read -r -a parsed_rule <<<"$rule_spec"
			run_cmd_dry ufw "${parsed_rule[@]}" ||
				log "WARN" "Failed to add rule: $rule_spec"
		fi
	done
...
validate_ufw_rule() {
	...
	[[ $rule =~ ^(allow|deny)\ (in|out)\ on\ [A-Za-z0-9]+\ to\ (any|[0-9./]+)\ port\ [0-9]+(\ proto\ (tcp|udp))?(\ comment\ .*)?$ ]] && return 0
