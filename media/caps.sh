...
get_fps() {
  local video=$1
  fps=$(ffprobe -v error -select_streams v -of default=noprint_wrappers=1:nokey=1 -show_entries stream=r_frame_rate "$video" | awk -F'/' '{print $1/$2}')
  handle_error
}
...
capture_frames() {
  ...
  local start=$2
  local end=$3
  ffmpeg -ss "$start" -to "$end" -i "$video" -vf "fps=$fps" -qscale:v 2 "$DATA_DIR/frame_%04d.png"
  handle_error
}
