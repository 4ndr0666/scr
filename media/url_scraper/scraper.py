...
def extract_pattern(url):
    ...
    idx = max(
        range(len(patterns)),
        key=lambda i: len(patterns[i][0]) if patterns[i][0].isdigit() else 0,
    )
...
def expand_numeric_pattern(base_pat, num0, width, ext, min_range, max_range):
    return [
        base_pat.format(num=str(i).zfill(width)) + ext
...
def brute_mode(url, min_range, max_range, download, headers, dry_run):
    ...
    try:
        ...
        print(
            f"Pattern: {base_pat.replace('{num}', '[N]')}, Range: {min_range}-{max_range}, Ext: {ext}"
        )
    except Exception as e:
        print(RED(f"Auto pattern failed: {e}"))
        sys.exit(1)
    urls = []
    for fname in expand_numeric_pattern(
        base_pat, num0, width, ext, min_range, max_range
    ):
        for f in all_exts_for(fname):
            # reconstruct dir
            upath = urlparse(url)
            url_base = (
                url[: url.rfind(upath.path)]
                + upath.path[: -(len(upath.path.split("/")[-1]))]
            )
...
def main():
    ap = argparse.ArgumentParser(
        description="Brute/recursive image enumerator/downloader (minimal, Arch/XDG)."
    )
    ...
    ap.add_argument(
        "--max", type=int, default=100, help="Max number in sequence (brute)"
    )
    ap.add_argument("--download", action="store_true", help="Download images")
    ap.add_argument("--user-agent", help="Custom User-Agent header")
    ap.add_argument(
        "--header", action="append", default=[], help="Add custom header (can repeat)"
    )
    ap.add_argument("--cookie", help="Cookie header")
    ap.add_argument(
        "--dry-run", action="store_true", help="Enumerate only, no download"
    )
