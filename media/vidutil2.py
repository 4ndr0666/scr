...
try:
    ...
    print("Prompt Toolkit is required for interactive prompts.\nInstall via: pip install prompt_toolkit")
    sys.exit(1)
...
def load_config():
    default_config = {
        "data_home": os.path.join(os.environ.get('XDG_DATA_HOME', os.path.join(os.environ['HOME'], '.local', 'share')), 'vidutil'),
...
def verify_and_repair_video_format(video: str) -> bool:
    ...
    if not re.search(r'\.(mp4|avi|mkv|mov|flv|wmv|webm|m4v|gif)$', video, re.IGNORECASE):
        ...
        return False
    try:
        result = subprocess.run(['ffprobe', video], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        if "moov atom not found" in result.stderr:
            ...
            repair_cmd = ["ffmpeg","-y","-i",video,"-c","copy","-movflags","faststart",temp_fixed_file]
...
def launch_svp_manager():
    ...
    if os.path.isfile(svp_manager_path) and os.access(svp_manager_path, os.X_OK):
        try:
            ...
            subprocess.Popen(['sudo', svp_manager_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            print_success("SVPManager launched successfully.")
        except Exception as ex:
            logging.warning(f"Could not invoke SVPManager automatically. {ex}")
            print_warning("Could not invoke SVPManager. Please run it manually if needed.")
    else:
        logging.warning(f"SVPManager binary not found or not executable at '{svp_manager_path}'.")
        print_warning(f"SVPManager binary not found or not executable at '{svp_manager_path}'. Please run it manually if needed.")
...
def screencaps_mode():
    ...
    try:
        ...
        all_videos = subprocess.run(find_cmd, stdout=subprocess.PIPE, text=True, check=True).stdout.strip().split('\n')
        if not all_videos or all_videos == ['']:
            print_warning("No video files found in current directory.")
            return
        fzf = shutil.which("fzf")
        if not fzf:
            print_warning("fzf is not installed. Please install fzf to use this feature.")
            return
        result = subprocess.run(["fzf","--prompt=Select video for screencaps>"], input='\n'.join(all_videos), stdout=subprocess.PIPE, text=True)
    ...
    output_dir=prompt(HTML(f'<ansicyan>Enter output directory for screenshots (default: {DEFAULT_SCREENCAP_DIR}): </ansicyan>'),style=style).strip()
    ...
    cmd=["ffmpeg","-y","-i",source_video,"-vf",f"fps=1/{interval}",os.path.join(output_dir,"screenshot_%04d.png")]
...
def clip_mode():
    ...
    try:
        ...
        if not fzf:
            print_warning("fzf is not installed. Please install fzf to use this feature.")
            return
        fzf_result=subprocess.run(["fzf","--prompt=Select video to clip>"], input='\n'.join(all_videos), stdout=subprocess.PIPE, text=True)
    ...
    print_warning("mpv+Lua integration is not implemented. Falling back to manual input.")
    ...
    if os.path.isfile(output_file):
        ...
        overwrite = prompt(HTML('<ansicyan>Do you want to overwrite it? [y/n]: </ansicyan>'), style=style).strip().lower()
        overwrite = sanitize_input(overwrite)
        if overwrite not in ['y', 'yes']:
            print_info("Clip creation aborted to prevent overwriting.")
            return

    cmd=["ffmpeg","-y","-ss",s,"-i",source_video,"-to",e,"-c","copy",output_file]
...
def normalize_video(file: str, temp_dir: str, target_resolution: str, target_framerate: float, denoise: bool=False) -> str:
    """
    ...
    audio_opt=["-an"]
    try:
        r=subprocess.run(["ffprobe","-v","error","-select_streams","a:0","-show_entries","stream=codec_type","-of","csv=p=0",file],
    ...
    cmd=["ffmpeg","-y","-i",file,"-c:v","libx264","-crf","23","-preset","medium"]+audio_opt
    if vf_chain:
        cmd+=["-vf",vf_chain]
...
def merge_group_videos(output_file:str, input_files:list, denoise:bool, motion:bool, user_options:dict, temp_dir:str):
    ...
    for f in input_files:
        nf=normalize_video(f,temp_dir,target_resolution,float(target_framerate),denoise=denoise)
        if nf:
    ...
    if merging_mode=='concat':
        ...
        with open(file_list_txt,'w') as ff:
            for n in normalized_files:
                ff.write(f"file '{n}'\n")
        cmd=["ffmpeg","-y","-f","concat","-safe","0","-i",file_list_txt,"-c:v","libx264","-preset","medium","-crf","23",
              "-pix_fmt","yuv420p","-c:a","aac","-b:a","128k",output_file]
        ...
        try:
            ...
            print_success(f"Video concatenation completed. Output saved to '{os.path.abspath(output_file)}'.")
        except Exception as e:
        ...
        cmd=["ffmpeg","-y"]+inputs+["-filter_complex",filters,"-map","[outv]","-c:v","libx264","-preset","medium","-crf","23","-pix_fmt","yuv420p",output_file]
        total_dur=get_total_duration(normalized_files)
        try:
            run_ffmpeg_with_progress(cmd,"Vertically stacking",total_dur)
            print_success(f"Vertical stacking completed. Output: '{os.path.abspath(output_file)}'.")
        except Exception as e:
        ...
        try:
            ...
            print_success(f"Grid merging completed. Output saved to '{os.path.abspath(output_file)}'.")
        except Exception as e:
        ...
        cmd=["ffmpeg","-y","-i",normalized_files[0],"-i",normalized_files[1],
             "-filter_complex","[0:v][1:v]hstack=inputs=2[outv]","-map","[outv]",
             "-c:v","libx264","-preset","medium","-crf","23","-pix_fmt","yuv420p",output_file]
        total_dur=get_total_duration(normalized_files)
        try:
            run_ffmpeg_with_progress(cmd,"Side-by-side merging",total_dur)
            print_success(f"Side-by-side merging done. '{os.path.abspath(output_file)}'.")
        except Exception as e:
...
def auto_merging_mode(valid_input_files:list, output_file:str, target_resolution:str, target_framerate:float, denoise:bool, motion:bool):
    """
    ...
    for f in valid_input_files:
        ...
        if props and 'width' in props and 'height' in props:
            try:
                w=int(props['width']);h=int(props['height'])
    ...
    for vf in sorted_files:
        nf=normalize_video(vf,temp_dir,target_resolution,float(target_framerate),denoise=denoise)
        if nf:
            normalized_files.append(nf)
    if not normalized_files:
    ...
    while len(normalized_files)>1:
        ...
        side_out=os.path.join(temp_dir,f"side_{os.path.basename(f1)}_{os.path.basename(f2)}.mp4")
        sidecmd=["ffmpeg","-y","-i",f1,"-i",f2,"-filter_complex","[0:v][1:v]hstack=inputs=2[outv]","-map","[outv]",
                 "-c:v","libx264","-preset","fast","-crf","23",side_out]
        try:
    ...
    with open(file_list,'w') as ff:
        for seg in segments:
            ff.write(f"file '{seg}'\n")
    cmd_concat=["ffmpeg","-y","-f","concat","-safe","0","-i",file_list,"-c:v","libx264","-preset","fast","-crf","23", output_file]
    total_dur=get_total_duration(segments)
    try:
        run_ffmpeg_with_progress(cmd_concat,"Concatenating auto segments", total_dur)
        print_success(f"Auto merging completed. Output saved to '{os.path.abspath(output_file)}'.")
...
def merge_videos(output_file:str, input_files:list, denoise:bool=False, motion:bool=False, user_options:dict=None):
    if not user_options:
        user_options={}
    merging_mode=user_options.get('merging_mode','concat')
    target_resolution=user_options.get('target_resolution','1920x1080')
    ...
    if merging_mode=='auto':
        auto_merging_mode(valid, output_file, target_resolution, float(target_framerate), denoise=denoise, motion=motion)
    else:
...
def merge_videos_mode():
    ...
    try:
        ...
        if not fzf:
            print_warning("fzf is not installed. Please install fzf to use this feature.")
            return
        result=subprocess.run(["fzf","--multi","--prompt=Select videos to merge>"],input='\n'.join(all_videos),stdout=subprocess.PIPE,text=True)
        if result.returncode!=0 or not result.stdout.strip():
    ...
    merging_choice=prompt(HTML('<ansicyan>Enter your choice (default: 1): </ansicyan>'),style=style).strip()
    merging_choice=sanitize_input(merging_choice)
    merging_mode_map={
        "1":"concat","2":"vstack","3":"grid","4":"side_by_side","5":"auto"
    }
    merging_mode=merging_mode_map.get(merging_choice,"concat")
    print_info(f"Selected merging mode: {merging_opts.get(merging_choice,merging_opts['1'])}")
    ...
    for k,v in container_map.items():
        print_formatted_text(f"{k}) {v}")
    cont_choice=prompt(HTML('<ansicyan>Enter your choice (default: mp4): </ansicyan>'),style=style).strip()
    ...
    for k,v in codec_map.items():
        print_formatted_text(f"{k}) {v}")
    c_choice=prompt(HTML('<ansicyan>Enter your choice (default: libx264): </ansicyan>'),style=style).strip()
    ...
    for k,v in q_map.items():
        print_formatted_text(f"{k}) {v}")
    q_choice=prompt(HTML('<ansicyan>Enter your choice (default: CRF): </ansicyan>'),style=style).strip()
    q_choice=sanitize_input(q_choice)
    quality=q_map.get(q_choice,"CRF")
    crf=None
    bitrate=None
    if quality=="CRF":
        crf_str=prompt(HTML('<ansicyan>Enter CRF value (lower better, default 23): </ansicyan>'),style=style).strip()
    ...
    preset_choice=prompt(HTML('<ansicyan>Enter your choice (default: medium): </ansicyan>'), style=style).strip()
    ...
    res_opts={"1":"1280x720","2":"1920x1080","3":"2560x1440","4":"3840x2160","5":"original"}
    for k,v in res_opts.items():
        print_formatted_text(f"{k}) {v}")
    r_choice=prompt(HTML('<ansicyan>Enter your choice (default: 1920x1080): </ansicyan>'),style=style).strip()
    ...
    motion_choice=prompt(HTML('<ansicyan>Do you want to apply motion interpolation via SVP4 after merging? [y/n] (default: n): </ansicyan>'),style=style).strip().lower()
    ...
    denoise_choice=prompt(HTML('<ansicyan>Do you want to apply denoising? (hqnlmeans/nlmeans)? [y/n] (default: n): </ansicyan>'),style=style).strip().lower()
    ...
    merge_videos(output_file, videos, denoise=denoise, motion=motion, user_options=user_options)

def main_menu():
    menu_options=["Screencaps","Clip","Merge Videos","Exit"]
    completer=WordCompleter(menu_options,ignore_case=True)
    while True:
        print_formatted_text(HTML('<ansicyan># --- // Vidutil2.py //</ansicyan>\n'),style=style)
        choice=prompt(HTML('<ansicyan>Menu: </ansicyan>'),completer=completer,style=style).strip().lower()
