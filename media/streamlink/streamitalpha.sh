...
run_streamlink() {
    ...
    while (( tries < MAX_RETRIES )); do
        ...
        streamlink "$url" "$q" \
          --output "$final_out" \
          $RETRY_STREAMS $HLS_OPTIONS $PROXY_OPTION $CACHE_OPTION \
          >"$final_log" 2>&1 &
...
schedule_stream() {
    ...
    echo -n "Quality: "; read -r q
    echo -n "Output base: "; read -r base
    echo -n "Cron schedule: "; read -r when
    [[ -z "$url" || -z "$q" || -z "$when" ]] && {
    ...
    ( crontab -l 2>/dev/null; echo "$when $cron_cmd" ) | crontab - || {
...
display_help() {
    echo "Usage: $0 [--url <stream_url>] [--quality <best|worst|720p60>] [--output <base_file>] [--help]"
    echo "Ex: $0 --url https://twitch.tv/somechannel --quality best --output myvid"
}
    ...
    while true; do
        ...
        case "$c" in
            ...
            4)
                ...
                ;;
            5)
                display_message info "Exiting. Goodbye!"
                exit 0
                ;;
            *)
