            ...
            with Progress(
                ...
                for future in asyncio.as_completed(tasks):
                    ...
                    progress.update(
                        task_id,
                        advance=1,
                        description=f"Scanning: Found {len(self.found_urls)} valid",
                    )
        ...
        try:
            if pattern:
                if "{num}" not in pattern:
                    raise ValueError("Pattern missing {num}")
...
def run_reddit_downloader():
    ...
    try:
        resp = httpx.get(
            ...
            headers={"User-Agent": "dorkmaster/6.0"},
            timeout=15,
        )
...
def telegram_megahunt(raw_query, max_posts=None):
    ...
    if not keywords:
        console.print(
            "[yellow]Cannot extract keywords from query for Telegram hunt.[/yellow]"
        )
        return []
    ...
    with httpx.Client(timeout=20, follow_redirects=True) as client:
        for variant in variants:
            for base in [f"https://t.me/s/{variant}", f"https://t.me/s/?q={variant}"]:
                try:
                    r = client.get(base, headers={"User-Agent": "Mozilla/5.0"})
                    ...
                    for post in posts[:max_posts]:
                        ...
                        text = (
                            text_div.get_text(strip=True, separator=" ")
                            if text_div
                            else ""
                        )
...
def run_searx(query, count=30):
    ...
    with httpx.Client(timeout=20, follow_redirects=True) as client:
        ...
        for base_url in pool:
            try:
                ...
                if results:
                    console.print(
                        f"[green]✓ SearxNG ({base_url}) — {len(results)} results[/green]"
                    )
                    return results[:count]
            except Exception:
                continue
    ...
    console.print(
        "[yellow]All SearxNG instances unreachable. No results returned.[/yellow]"
    )
    return []
...
def analyze_target(url):
    ...
    try:
        resp = httpx.get(
            url,
            timeout=15,
            headers={"User-Agent": "Mozilla/5.0 (Ψ-NSFW-Hunter/2025)"},
...
class GalleryEnumerator:
    """Universal temple for CDN bucket + WP sitemap → media enumeration.
    Hardened: Conditional WP sitemap, always brute numeric bucket,
    intermediate saves on every step, dynamic filename, full-res upgrade."""

    def __init__(self):
        self.headers = httpx.Headers(
            {"User-Agent": "Mozilla/5.0 (Compatible; 4ndr0666-GalleryHunter/2026)"}
        )
        self.found_media = set()
        self.output_dir = Path(DOWNLOADS_DIR) / "temple_results"
        self.output_dir.mkdir(parents=True, exist_ok=True)
        console.print(f"[dim]Temple output directory: {self.output_dir}[/dim]")

    async def fetch_sitemap(self, domain):
        console.print(f"[cyan]Fetching WP sitemaps from {domain}...[/cyan]")
        sitemap_url = f"https://{domain}/wp-sitemap.xml"
        try:
            async with httpx.AsyncClient(headers=self.headers, timeout=20) as client:
                ...
                console.print(f"[green]→ Found {len(sitemaps)} sitemaps[/green]")
                return sitemaps
        except Exception as e:
            console.print(f"[red]Sitemap fetch failed: {e}[/red]")
            return []

    async def extract_gallery_urls(self, sitemaps):
        urls = []
        async with httpx.AsyncClient(headers=self.headers, timeout=20) as client:
            for s in sitemaps:
                if "posts-post" not in s.lower():
                    continue
                try:
                    ...
                    new_urls = [
                        ...
                        if any(
                            x in loc.text.lower()
                            for x in ["gallery", "picture", "video", "photoshoot"]
                        )
                    ]
                    urls.extend(new_urls)
                    console.print(
                        f"[green]→ Extracted {len(new_urls)} gallery URLs from {s}[/green]"
                    )
                except Exception as e:
                    console.print(f"[yellow]Skipped sitemap {s}: {e}[/yellow]")
                    continue
        console.print(
            f"[bold green]Total galleries discovered: {len(urls)}[/bold green]"
        )
        return urls

    async def scrape_gallery_media(self, gallery_url):
        console.print(
            f"[blue]Scraping gallery (full-res upgrade): {gallery_url}[/blue]"
        )
        try:
            async with httpx.AsyncClient(headers=self.headers, timeout=20) as client:
                ...
                for tag in soup.find_all(["img", "video", "source", "a"]):
                    for attr in [
                        ...
                        if not src or not any(
                            c in src for c in ["cdn.", "uecdn.es", "celebhottie"]
                        ):
                            continue
                        ...
                        original_src = src
                        src = re.sub(
                            r"[-_](thumb|thumbnail|icon|small|preview|tn|low|mini)\b",
                            "",
                            src,
                            flags=re.I,
                        )
                        src = re.sub(r"[-_](\d{2,4}x\d{2,4})", "", src)
                        src = re.sub(r"[-_]\d{2,4}w", "", src)

                        if src != original_src:
                            cdn_links.append(original_src)
                ...
                seen = set()
                unique_links = [
                    link for link in cdn_links if not (link in seen or seen.add(link))
                ]

                self.found_media.update(unique_links)
                console.print(
                    f"[green]→ {len(unique_links)} media links extracted (full-res upgraded)[/green]"
                )
                self._save_current_results()
        except Exception as e:
            console.print(f"[red]Gallery scrape failed for {gallery_url}: {e}[/red]")

    async def brute_numeric_bucket(self, cdn_base, id_range, filename_patterns=None):
        ...
        console.print(
            f"[cyan]Bruting numeric bucket {cdn_base} range {id_range}...[/cyan]"
        )
        candidates = []
        for i in range(id_range[0], id_range[1] + 1):
            for name in filename_patterns:
                for ext in [".jpg", ".jpeg", ".png", ".webp", ".mp4", ".mov"]:
                    url = f"{cdn_base.rstrip('/')}/{i}/{name}{ext}".replace("//", "/")
                    candidates.append(url)

        async with httpx.AsyncClient(headers=self.headers, timeout=8) as client:
            tasks = [client.head(u, follow_redirects=True) for u in candidates]
            for resp in await asyncio.gather(*tasks, return_exceptions=True):
                if isinstance(resp, httpx.Response) and resp.status_code == 200:
                    self.found_media.add(str(resp.url))
                    console.print(f"[green][200] {resp.url}[/green]")

        console.print(
            f"[bold green]Bucket brute complete — {len(self.found_media)} total media[/bold green]"
        )
        self._save_current_results()
    ...
    def _save_current_results(self):
        ...
        save_file = (
            self.output_dir / f"media_{self.output_dir.parent.name}_{timestamp}.txt"
        )  # dynamic
        with open(save_file, "w", encoding="utf-8") as f:
            for link in sorted(self.found_media):
                f.write(link + "\n")
        console.print(
            f"[dim]Intermediate save: {len(self.found_media)} links → {save_file.name}[/dim]"
        )

    async def run_full_enumeration(
        self, domain, cdn_base, numeric_range=(1000, 8000), filename_patterns=None
    ):
        console.print(f"[dim]Temple output directory: {self.output_dir}[/dim]")

        # Conditional WP sitemap (skip on pure CDNs like elmundo)
        is_wp_like = any(
            k in domain.lower() for k in ["wordpress", "blog", "news", "celebhottie"]
        )
        if is_wp_like:
            sitemaps = await self.fetch_sitemap(domain)
            if sitemaps:
                galleries = await self.extract_gallery_urls(sitemaps)
                for g in galleries[:150]:
                    await self.scrape_gallery_media(g)

        # Always run numeric brute (core for elmundo-style CDNs)
        await self.brute_numeric_bucket(cdn_base, numeric_range, filename_patterns)

        # Final save with dynamic domain name
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        save_file = self.output_dir / f"media_{domain}_{timestamp}.txt"
        with open(save_file, "w", encoding="utf-8") as f:
            for link in sorted(self.found_media):
                f.write(link + "\n")

        console.print(
            f"[bold green]✓ Final save: {len(self.found_media)} media links to {save_file}[/bold green]"
        )

        if self.found_media and Confirm.ask("Download all discovered media now?"):
            download_with_aria2c(
                list(self.found_media), self.output_dir / f"download_{timestamp}"
            )

        return sorted(list(self.found_media))
...
def settings_menu(current_config):
    ...
    while True:
        ...
        choice = prompt(
            "Edit setting (name), [S]ave & Back, [R]eset, [B]ack without saving"
        )
        if choice.lower() == "b":
            ...
            console.print(
                "[yellow]Config reset to defaults. Choose 'Save & Back' to confirm.[/yellow]"
            )
        elif choice in temp_config:
            if isinstance(temp_config[choice], (list, dict)):
                console.print(
                    f"[yellow]Please edit '{choice}' directly in {CONFIG_FILE}[/yellow]"
                )
                continue
            ...
            try:
                ...
                console.print(
                    f"[green]'{choice}' set to: {temp_config[choice]}[/green]"
                )
            except ValueError:
...
def main_palette():
    ...
    while True:
        ...
        table.add_row("12", "Universal Gallery Enumeration Temple")
        ...
        if sel == "12":
            domain = prompt("Domain (e.g. celebhottie.com)")
            cdn_base = prompt("CDN base (e.g. https://cdn.celebhottie.com)")
            enum = GalleryEnumerator()
            media = asyncio.run(enum.run_full_enumeration(domain, cdn_base))
            console.print(
                f"[bold green]Universal Temple complete — {len(media)} media links mapped and saved[/bold green]"
            )
            if media and Confirm.ask("Download all discovered media now?"):
                download_with_aria2c(
                    media,
                    Path(DOWNLOADS_DIR)
                    / f"download_{domain}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}",
                )
            continue
        ...
        if sel == "1":
            ...
            node = MycelialNode(
                {"base_url": url, "found_count": len(enum.found_urls)}, "image_brute"
            )
            ...
            node = MycelialNode(
                {"start_url": url, "found_count": len(enum.found_urls)}, "image_crawl"
            )
            session.add_root(node)
            ...
            if sel_plugin.isdigit():
                ...
                if 0 <= idx < len(plugin_names):
                    ...
                    if hasattr(module, "run"):
                        try:
                            console.print(
                                f"[bold green]Running {pname}...[/bold green]"
                            )
                        ...
                        console.print(
                            f"[yellow]Plugin '{pname}' has no run() method.[/yellow]"
                        )
                elif idx != -1:
