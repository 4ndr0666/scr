...
def telegram_megahunt(raw_query, max_posts=50):
    ...
    # X/Web Pivot Fallback
    console.print("[bold yellow]Telegram sterile – pivoting to X/web mirrors[/bold yellow]")
    fallback_query = f"{clean} telegram OR discord site:x.com OR site:reddit.com"
    fallback_results = run_searx(fallback_query, count=10)  # Recursive safe – depth limited
    for r in fallback_results:
        if "t.me" in r.get("url", "") or "discord.gg" in r.get("url", ""):
            results.append({"url": r["url"], "title": r.get("title", "X/Web Mirror"), "content": r.get("content", "")})
    if results:
        console.print(f"[bold purple]X Pivot Success – {len(results)} mirrors ripped[/bold purple]")
    return results or [{"url": "VOID", "title": "No veins found", "content": ""}]
...
def run_searx(query, categories=["general"], count=30):
    ...
    if not results or (len(results) == 1 and "VOID" in results[0]["url"]):
        browser_fallback(query)
        return []
    return results

def browser_fallback(query):
    clean = re.sub(r'[\(\)\"OR]', '', query, flags=re.I).strip()
    google_url = f"https://www.google.com/search?q={quote_plus(clean)}"
    console.print(f"[bold red]All grids dead – opening dork in browser[/bold red]")
