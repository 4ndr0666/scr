...
def generate_gallery_dl_config(
    ...
    placeholder_url = base_url.replace(
        str(start_num).zfill(zero_padding), "{num:0" + str(zero_padding) + "}"
    )
...
def main():
    ...
    base_url = input(
        "Enter the base URL (with the numeric part included, e.g. '...-46.jpg'): "
    ).strip()
    ...
    zero_padding = int(
        input("Enter the zero-padding size (e.g., 2 for '01', 3 for '001'): ")
    )
    ...
    sleep_interval = [
        float(
            input(
                "Enter minimum sleep interval between requests (default 2.0): "
            ).strip()
            or 2.0
        ),
        float(
            input(
                "Enter maximum sleep interval between requests (default 4.8): "
            ).strip()
            or 4.8
        ),
    ]
