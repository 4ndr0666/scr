...
def generate_gallery_dl_config(
    base_url, start_num, end_num, zero_padding,
    rate, retries, timeout,
    postprocessors, sleep_interval,
    proxy, no_part, no_skip,
    write_log, write_unsupported, write_pages,
    resolve_urls, get_urls,
    cookies, headers,
    additional_options, 
    output_directory,
    config_file
    ...
    config = {
        ...
        "downloader": {
            ...
            "part-directory": "/tmp/.download/" if not no_part else "",  # Store partial downloads temporarily
        ...
        "output": {
            ...
            "unsupportedfile": {
                "path": os.path.expanduser(write_unsupported) if write_unsupported else None,
                "mode": "a",
...
def choose_header_preset():
    ...
    if choice == '1':
        ...
        custom_headers_input = input("Enter custom headers in JSON format (e.g., {\"Header-Name\": \"Header-Value\"}): ").strip()
...
def prompt_for_additional_options():
    ...
    additional_options = {}
    while True:
        add_option = input("Do you want to add an additional option? (y/n): ").strip().lower()
        if add_option == 'n':
            break
        elif add_option != 'y':
            print("Please enter 'y' or 'n'.")
...
def main():
    ...
    base_url = input("Enter the base URL (with a placeholder for numbering, e.g., 'https://www.x.com/image{num}.jpg'): ").strip()
    ...
    if "{num}" not in base_url:
        print(f"{LIGHTRED}Error: The base URL must contain the placeholder '{{num}}' for numbering.{NC}")
        sys.exit(1)
    ...
    if end_num < start_num:
        print(f"{LIGHTRED}Error: Ending number ({end_num}) cannot be less than starting number ({start_num}).{NC}")
        sys.exit(1)

    # Ask for zero-padding size (e.g., 01, 02, ...)
    zero_padding_input = input("Enter the zero-padding size (default: 2): ").strip() or "2"
    ...
    retries_input = input("Enter the number of retries for failed downloads (default: 3): ").strip() or "3"
    retries = validate_numeric_input(retries_input, "Number of retries")

    timeout_input = input("Enter the timeout for a download to be considered failed (default: 8.0 seconds): ").strip() or "8.0"
    timeout = validate_numeric_input(timeout_input, "Timeout")

    # Proxy settings
    proxy = input("Enter proxy URL if needed (e.g., http://proxyserver:port) [Leave blank to skip]: ").strip() or None

    # Flags for downloader options
    no_part_input = input("Do you want to disable .part files? (y/n, default: n): ").strip().lower() or "n"
    no_part = True if no_part_input == 'y' else False

    no_skip_input = input("Do you want to disable skipping existing files? (y/n, default: n): ").strip().lower() or "n"
    ...
    # Sleep intervals
    min_sleep_input = input("Enter minimum sleep interval between requests (default: 2.0 seconds): ").strip() or "2.0"
    min_sleep = validate_numeric_input(min_sleep_input, "Minimum sleep interval")

    max_sleep_input = input("Enter maximum sleep interval between requests (default: 4.8 seconds): ").strip() or "4.8"
    max_sleep = validate_numeric_input(max_sleep_input, "Maximum sleep interval")

    if max_sleep < min_sleep:
        print(f"{LIGHTRED}Error: Maximum sleep interval ({max_sleep}) cannot be less than minimum sleep interval ({min_sleep}).{NC}")
        sys.exit(1)
    ...
    write_log = input("Enter log file path if you want to write logs [Leave blank to skip]: ").strip() or None
    write_unsupported = input("Enter unsupported URLs log file path [Leave blank to skip]: ").strip() or None
    write_pages_input = input("Do you want to write downloaded intermediary pages for debugging? (y/n, default: n): ").strip().lower() or "n"
    write_pages = True if write_pages_input == 'y' else False

    # Resolve URLs and Get URLs options
    resolve_urls_input = input("Do you want to resolve intermediary URLs? (y/n, default: n): ").strip().lower() or "n"
    resolve_urls = True if resolve_urls_input == 'y' else False

    get_urls_input = input("Do you want to print URLs instead of downloading? (y/n, default: n): ").strip().lower() or "n"
    ...
    # Output directory
    output_directory = input("Enter the download directory path (default: current directory): ").strip() or "."

    # Configuration file name
    config_file = input("Enter the configuration file name (default: gallery-dl-config.json): ").strip() or "gallery-dl-config.json"
    ...
    run_now_input = input("Do you want to run gallery-dl immediately with the generated configuration? (y/n, default: y): ").strip().lower() or "y"
    if run_now_input == 'y':
        ...
        print("Configuration generation completed. You can run gallery-dl manually when ready.")
