...
while (( $# )); do
  case "${1:-}" in
    ...
    --verbose) VERBOSE=1; FFLOGLEVEL="info"; shift ;;
    --full) FULL_CONCAT=1; shift ;; # New flag to enable full concatenation
    *) ARGS+=("$1"); shift ;;
  esac
done
set -- "${ARGS[@]:-}"
...
run_config_setup() {
    ...
    mkdir -p "$PROJECT_ROOT" || { echo "Error: Could not create project root directory '$PROJECT_ROOT'."; exit 1; }
...
if [[ -n "${CONFIG_TMPDIR_PATH:-}" ]]; then
    ...
    mkdir -p "$TMPDIR" || { echo "Error: Could not create configured TMPDIR '$TMPDIR'. Please check permissions."; exit 1; }
...
process_image() {
  ...
  [[ -z "${W:-}" || -z "${H:-}" ]] && { echo "Identify failed for image: $in"; return; }
