...
#         (Default output: <input_basename>_slowmo.mp4; default factor: 3, target fps: 60)
#   fix     [<input>] <output> [-an]
...
MAX_1080=false    # New flag: enforce max 1080p scaling in process
...
moov_fallback() {
  ...
  ffmpeg -y -i "$in_file" -c:v "$ADV_CODEC" -crf "$ADV_CRF" -preset medium $audio_opts \
...
## Run FFMPEG: Robust wrapper calling FFmpeg and returning its exit code.
run_ffmpeg() {
  set +e
  ffmpeg "$@"
  local status=$?
...
## Advanced: Interactive advanced mode prompt
advanced_prompt() {
  if [ "$ADVANCED_MODE" = true ]; then
    echo "# --- // ADVANCED MODE //"
    local cont
    read -r -p "Extension? [mp4/mkv/mov] (default=mkv): " cont
    if [ -z "$cont" ]; then
      cont="mkv"
    fi
    ADV_CONTAINER="$cont"
    ...
    local fpsin
    read -r -p "Fps? (24/30/60/120/240) (default=60): " fpsin
    if [ -z "$fpsin" ]; then
      fpsin="60"
    fi
    ADV_FPS="$fpsin"
    ...
    local choice
    read -r -p "Codec? 1=libx264, 2=libx265 (default=1): " choice
    if [ "$choice" = "2" ]; then
      ADV_CODEC="libx265"
    else
      ADV_CODEC="libx264"
    ...
    local crfval
    read -r -p "CRF? (0=lossless, default=18): " crfval
    if [ -z "$crfval" ]; then
      crfval="18"
    fi
    ADV_CRF="$crfval"
...
is_interlaced() {
  ...
  local f_order
  f_order="$(ffprobe -v error -select_streams v:0 -show_entries stream=field_order \
    -of default=noprint_wrappers=1:nokey=1 "$file" 2>/dev/null || echo "")"
...
check_dts_for_file() {
  ...
  local prev=0
  local prob=0
  ffprobe -v error -select_streams v -show_entries frame=pkt_dts_time \
    -of csv=p=0 "$file" 2>/dev/null | while read -r line; do
      line="$(echo "$line" | tr -d ',' | xargs)"
      if ! echo "$line" | grep -Eq '^[0-9]+(\.[0-9]+)?$'; then
        continue
      fi
      awk "BEGIN {if ($line < $prev) exit 0; else exit 1}" >/dev/null 2>&1
      if [ $? -eq 0 ]; then
...
fix_dts() {
  ...
  run_ffmpeg -y -fflags +genpts -i "$file" \
    -c:v copy $audio_opts -movflags +faststart "$tmpf" >/dev/null 2>&1
  if [ $? -ne 0 ] || [ ! -f "$tmpf" ]; then
    run_ffmpeg -y -fflags +genpts -i "$file" \
      -c:v "$ADV_CODEC" -crf "$ADV_CRF" -preset medium $audio_opts "$tmpf" >/dev/null 2>&1
    if [ $? -ne 0 ] || [ ! -f "$tmpf" ]; then
      echo "❌ fix_dts: Unable to fix DTS for $file" >&2
      rm -f "$tmpf"
      return 1
    fi
  fi
...
ensure_dts_correct() {
  ...
  check_dts_for_file "$file"
  if [ $? -ne 0 ]; then
    local fixed
    fixed="$(fix_dts "$file")"
    if [ $? -ne 0 ] || [ ! -f "$fixed" ]; then
...
auto_clean() {
  ...
  if [ "$CLEAN_META_DEFAULT" = true ] && [ -f "$file" ]; then
    ...
    run_ffmpeg -y -i "$file" -map_metadata -1 -c copy "$tmpf" >/dev/null 2>&1
    if [ $? -eq 0 ] && [ -f "$tmpf" ]; then
      mv "$tmpf" "$file"
...
## Process: Losslessly and intelligent downscale (1080p)
#### Direct transmux (lossless) if possible, re-encode if required.
#### -P flag forces downscaling to a maximum of 1080p.
...
cmd_process() {
  ...
  #### Determine if re-encoding is required
  local scale_filter=""
  local do_encode=false
  #### Use advanced options if user explicitly set resolution/fps or forced_fps exists
  if [ "$ADV_RES" != "1920x1080" ] || [ "$ADV_FPS" != "60" ] || [ -n "$forced_fps" ]; then
    scale_filter="scale=${ADV_RES}:force_original_aspect_ratio=decrease"
    do_encode=true
  fi

  #### Check container mismatch: if input is webm or not matching desired container.
  local cont
  ...
  #### If -P (max1080) flag is set, check input height and force downscale if needed.
  if [ "$MAX_1080" = true ]; then
    ...
    if [ -n "$orig_height" ] && [ "$orig_height" -gt 1080 ] 2>/dev/null; then
      verbose_log "cmd_process => Input height ($orig_height) > 1080; enforcing max 1080p"
  ...
  if [ "$do_encode" = false ]; then
    verbose_log "cmd_process => Attempting direct copy (transmux)"
    run_ffmpeg -y -i "$input" -c copy -movflags +faststart "$output" >/dev/null 2>&1
    if [ ! -f "$output" ]; then
...
composite_group() {
  ...
  if [ "$count" -eq 1 ]; then
    ...
    run_ffmpeg -y -i "${files[0]}" -i "${files[1]}" \
      -filter_complex "hstack=inputs=2, pad=${w}:${h}:((ow-iw)/2):((oh-ih)/2):black" \
      -c:v "$ADV_CODEC" -crf "$ADV_CRF" -preset medium -pix_fmt "$ADV_PIX_FMT" $(get_audio_opts) \
      "$c_file" >/dev/null 2>&1
  elif [ "$count" -eq 3 ]; then
    #### Arrange two side-by-side on top and one centered at the bottom.  
    run_ffmpeg -y -i "${files[0]}" -i "${files[1]}" -i "${files[2]}" \
      -filter_complex "vstack=inputs=3" \
      -c:v "$ADV_CODEC" -crf "$ADV_CRF" -preset medium -pix_fmt "$ADV_PIX_FMT" $(get_audio_opts) \
      "$c_file" >/dev/null 2>&1
  elif [ "$count" -eq 4 ]; then
    #### Arrange in a 2x2 grid.  
    run_ffmpeg -y -i "${files[0]}" -i "${files[1]}" -i "${files[2]}" -i "${files[3]}" \
    ...
    run_ffmpeg -y $(for ff in "${files[@]}"; do echo -i "$ff"; done) \
  ...
  if ! moovCheck "$c_file"; then
    verbose_log "composite_group: moov missing; invoking fallback."
...
cmd_merge() {
  ...
  for f in "${files[@]}"; do
    ...
    res="$(ffprobe -v error -select_streams v:0 -show_entries stream=width,height \
      -of csv=p=0:s=x "$fixd" 2>/dev/null)"
    local h
  ...
  for f in "${files[@]}"; do
    ...
    if [ "$cont_chk" = "webm" ]; then
      run_ffmpeg -y -i "$f" -r "$tf" \
        -vf "pad=${TARGET_WIDTH}:${TARGET_HEIGHT}:((TARGET_WIDTH-iw)/2):((TARGET_HEIGHT-ih)/2):black" \
        -c:v "$ADV_CODEC" -crf "$ADV_CRF" -preset medium $(get_audio_opts) -movflags +faststart \
        "$tmpf" >/dev/null 2>&1
    else
      run_ffmpeg -y -i "$f" -r "$tf" \
        -vf "pad=${TARGET_WIDTH}:${TARGET_HEIGHT}:((TARGET_WIDTH-iw)/2):((TARGET_HEIGHT-ih)/2):black" \
        -c:v "$ADV_CODEC" -crf "$ADV_CRF" -preset medium $(get_audio_opts) \
        "$tmpf" >/dev/null 2>&1
    fi
    if [ $? -ne 0 ] || [ ! -f "$tmpf" ]; then
    ...
    if ! moovCheck "$tmpf"; then
      verbose_log "merge: Moov missing for $f; invoking fallback."
  ...
  run_ffmpeg -y -avoid_negative_ts make_zero -f concat -safe 0 -i "$concat_list" -c copy "$output" >/dev/null 2>&1
...
cmd_looperang() {
  ...
  fps="$(ffprobe -v error -select_streams v:0 -show_entries stream=r_frame_rate \
    -of default=noprint_wrappers=1:nokey=1 "$input" 2>/dev/null || echo "25")"
  ...
  ffmpeg -y -framerate "$fps" -i "$fwd_dir/frame-%06d.jpg" \
    -c:v "$ADV_CODEC" -crf "$ADV_CRF" -preset medium -pix_fmt "$ADV_PIX_FMT" "$fwd_temp" >/dev/null 2>&1
  ffmpeg -y -framerate "$fps" -i "$rev_dir/frame-%06d.jpg" \
  ...
  ffmpeg -y -avoid_negative_ts make_zero -f concat -safe 0 -i "$c_list" -c copy "$output" >/dev/null 2>&1
...
## Slowmo: Lossless PTS manipulation with optional interpolation
...
cmd_slowmo() {
  ...
  if [ -z "$input" ] || [ ! -f "$input" ]; then
    echo "❌ Slowmo: Input file required."
    exit 1
  fi
  if [ -z "$output" ]; then
    local b
  ...
  if [ "$ADV_MULTIPASS" = "true" ]; then
    ffmpeg -y -i "$input" "${fps_cmd[@]}" -filter:v "setpts=${factor}*PTS" \
      -c:v "$ADV_CODEC" -b:v "$ADV_BR" -preset medium -pass 1 -an -f mp4 /dev/null >/dev/null 2>&1
    ffmpeg -y -i "$input" "${fps_cmd[@]}" -filter:v "setpts=${factor}*PTS" \
      -c:v "$ADV_CODEC" -b:v "$ADV_BR" -preset medium -pass 2 $audio_opts -movflags +faststart \
      "$output" >/dev/null 2>&1
  else
    ffmpeg -y -i "$input" "${fps_cmd[@]}" -filter:v "setpts=${factor}*PTS" \
      -c:v "$ADV_CODEC" -crf "$ADV_CRF" -preset medium $audio_opts "$output" >/dev/null 2>&1
  fi
  ...
  if ! moovCheck "$output"; then
    echo "❌ Slowmo: Moov atom missing in $output"
    exit 1
  fi
...
cmd_fix() {
  ...
  if [ "$BULK_MODE" = true ]; then
    ...
    if [ "$c" = "f" ]; then
      ...
      for f in "$tmpd"/*; do
        ...
        local audio_opts
        audio_opts="$(get_audio_opts)"
        run_ffmpeg -y -fflags +genpts -i "$f" -c:v copy $audio_opts -movflags +faststart "$out" >/dev/null 2>&1
        if [ $? -ne 0 ] || [ ! -f "$out" ]; then
          run_ffmpeg -y -fflags +genpts -i "$f" -c:v "$ADV_CODEC" -crf "$ADV_CRF" \
            -preset veryfast $audio_opts "$out" >/dev/null 2>&1
        fi
        auto_clean "$out"
      done
      rm -rf "$tmpd"
  ...
  local audio_opts
  audio_opts="$(get_audio_opts)"
  run_ffmpeg -y -fflags +genpts -i "$input" -c:v copy $audio_opts -movflags +faststart "$output" >/dev/null 2>&1
  if [ $? -ne 0 ] || [ ! -f "$output" ]; then
    run_ffmpeg -y -fflags +genpts -i "$input" -c:v "$ADV_CODEC" -crf "$ADV_CRF" \
      -preset veryfast $audio_opts "$output" >/dev/null 2>&1
  fi
  auto_clean "$output"
...
cmd_timeline() {
  ...
  local audio_opts
  audio_opts="$(get_audio_opts)"
  run_ffmpeg -y -i "$input" -ss "$st" -to "$et" $audio_opts -c:v "$ADV_CODEC" \
    -crf "$ADV_CRF" -preset medium "$output" >/dev/null 2>&1
  auto_clean "$output"
...
cmd_cleanmeta() {
  ...
  if [ -z "$input" ] || [ -z "$output" ]; then
    if [ "$#" -eq 1 ]; then
      output="$1"
      if command_exists fzf; then
        echo "➡️ No input for clean; launching fzf..."
        input="$(fzf)"
        if [ -z "$input" ]; then
          echo "❌ No file selected."
          exit 1
  ...
  if [ "$BULK_MODE" = true ]; then
    ...
    if [ "$c" = "f" ]; then
      ...
      for f in "$tmpd"/*; do
        ...
        local audio_opts
        audio_opts="$(get_audio_opts)"
        run_ffmpeg -y -i "$f" -map_metadata -1 -c:v copy $audio_opts -movflags +faststart "$out" >/dev/null 2>&1
        if [ $? -ne 0 ]; then
          run_ffmpeg -y -i "$f" -map_metadata -1 -c:v "$ADV_CODEC" -crf "$ADV_CRF" \
            -preset medium $audio_opts "$out" >/dev/null 2>&1
        fi
      done
      rm -rf "$tmpd"
  ...
  local audio_opts
  audio_opts="$(get_audio_opts)"
  run_ffmpeg -y -i "$input" -map_metadata -1 -c:v copy $audio_opts -movflags +faststart "$output" >/dev/null 2>&1
  if [ $? -ne 0 ]; then
    run_ffmpeg -y -i "$input" -map_metadata -1 -c:v "$ADV_CODEC" -crf "$ADV_CRF" \
      -preset medium $audio_opts "$output" >/dev/null 2>&1
  fi
