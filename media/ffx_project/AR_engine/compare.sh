...
#   ffx-composite -b 4 -o my_output.mp4 clip1.mp4 clip2.mp4 clip3.mp4
#
# Options:
#   -o FILE    Final output path (default: composite.mp4 in cwd)
#   -b SIZE    Force a specific batch size (1-4). Overrides randomness.
#   -h         Show this help
#
# Pipeline:
...
# =============================================================================
usage() {
    printf 'Usage: %s [-o OUTPUT] [-b BATCH_SIZE] FILE [FILE ...]\n\n' "$(basename "$0")"
    ...
    printf '  -b SIZE    Force batch size (1-4). If omitted, batches are randomly sized.\n'
...
L_IDS+=("top2_bot1");    L_SLOTS+=("960:540 960:540 1920:540");
...
L_IDS+=("left1_right2"); L_SLOTS+=("960:1080 960:540 960:540");
...
L_IDS+=("left2_right1"); L_SLOTS+=("960:540 960:540 960:1080");
...
L_IDS+=("grid2x2");      L_SLOTS+=("960:540 960:540 960:540 960:540");
...
L_IDS+=("row4");         L_SLOTS+=("480:1080 480:1080 480:1080 480:1080");
...
select_layout() {
    ...
    for cidx in "${cands[@]}"; do
        ...
        if (( _LAST_SCORE > best_score )); then
            best_score=$_LAST_SCORE; top_cands=("$cidx")
...
composite_batch() {
    ...
    # Gap Mitigation: Dynamic amix insertion prevents audio loss on inputs > 1
    if (( n > 1 )); then
        local audio_mix=""
        for ((i=0; i<n; i++)); do
            audio_mix+="[${i}:a]"
        done
        audio_mix+="amix=inputs=${n}:duration=longest[aout]"
        fc="${fc};${audio_mix}"
        a_map="[aout]"
    fi
