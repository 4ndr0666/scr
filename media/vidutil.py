...
def get_video_properties(file):
    try:
        ...
        logging.warning(f"Failed to get properties for '{file}'. Error: {get_error_output(e)}")
        return None
...
def verify_and_repair_video_format(video):
    ...
    try:
        result = subprocess.run(['ffprobe', video], stderr=subprocess.PIPE, stdout=subprocess.PIPE, text=True)
...
def normalize_video(file, temp_dir, target_resolution, target_framerate=None, codec='libx264', crf=23, preset='medium', tune=None, denoise=False, motion_interpolation=False):
    ...
    if total_duration == 0.0:
        ...
        try:
            ...
            logging.warning(f"Failed to normalize '{file}'. Skipping. Error: {get_error_output(e)}")
            return None
...
def merge_videos(output_file, input_files, denoise=False, user_options=None):
    ...
    merging_mode = user_options.get('merging_mode', 'concat')  # Default to 'concat' if not specified
    ...
    if merging_mode == "auto":
        ...
        while files_to_process:
            ...
            merge_group_videos(group_output, group_files, denoise, group_user_options, temp_dir)
        ...
        if len(segment_videos) == 1:
            ...
            logging.info(f"Auto merging completed successfully. Output file: '{output_file}'.")
            print_success(f"Auto merging completed successfully. Output file: '{output_file}'.")
        else:
...
def merge_group_videos(output_file, input_files, denoise, user_options, temp_dir=None):
    ...
    if merging_mode == "concat":
        ...
        for cols in range(1, num_inputs + 1):
            ...
            if min_aspect_ratio_diff is None or aspect_ratio_diff < min_aspect_ratio_diff:
        ...
        if max_height:
            for idx in range(2):
                filters += f'[{idx}:v] setpts=PTS-STARTPTS, scale=-1:{max_height} [v{idx}]; '
        else:
            for idx in range(2):
                filters += f'[{idx}:v] setpts=PTS-STARTPTS [v{idx}]; '

        filters += f'[v0][v1] hstack=inputs=2 [outv]'
        ...
        try:
            run_ffmpeg_with_progress(cmd, description, total_duration)
            logging.info(f"Side-by-side merging completed successfully. Output file: '{output_file}'.")
            print_success(f"Side-by-side merging completed successfully. Output file: '{output_file}'.")
        except Exception as e:
...
def merge_videos_mode():
    ...
    print_info(f"Selected merging mode: {merging_options.get(merging_choice, merging_options['1'])}")
    ...
    for key, value in container_options.items():
        print_formatted_text(f"{key}) {value}")
    container_choice = prompt(HTML('<ansicyan>Enter your choice (default: mp4): </ansicyan>'), style=style).strip()
    ...
    for key, value in codec_options.items():
        print_formatted_text(f"{key}) {value}")
    codec_choice = prompt(HTML('<ansicyan>Enter your choice (default: libx264): </ansicyan>'), style=style).strip()
    ...
    for key, value in quality_options.items():
        print_formatted_text(f"{key}) {value}")
    quality_choice = prompt(HTML('<ansicyan>Enter your choice (default: CRF): </ansicyan>'), style=style).strip()
    ...
    for key, value in resolution_options.items():
        print_formatted_text(f"{key}) {value}")
    resolution_choice = prompt(HTML('<ansicyan>Enter your choice (default: 1920x1080): </ansicyan>'), style=style).strip()
