...
Handles user interaction, command-line arguments, 
and calls enumerator.py + integration.py for extended capabilities:

  - Browser opening
  - Downloading with aria2
  - Storing enumerated URLs in a local file
...
Dependencies:
    ...
    - aria2 (installed on system, if using the download feature)
"""
...
def main():
    ...
    parser.add_argument("--open-browser", action="store_true", help="Open enumerated images in new browser tabs.")
    parser.add_argument("--download-aria2", metavar="DOWNLOAD_DIR", type=str,
                        help="Download enumerated images using aria2 to the specified directory.")
    parser.add_argument("--store", metavar="FILENAME", type=str,
    ...
    if not args.u:
        enumerator.color_print("Error: No URL provided. Use -u <URL> or -m for the menu.", enumerator.LIGHT_RED)
        show_help()
        sys.exit(1)
