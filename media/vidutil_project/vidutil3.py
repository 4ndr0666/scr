...
def normalize_video(file, temp_dir, target_resolution, target_framerate=None, codec='libx264', crf=23, preset='medium', tune=None, denoise=False, scale_filter=None):
    if not verify_and_repair_video_format(file):
...
def merge_videos(output_file, input_files, denoise=False):
    ...
    options_fps = {"1": "24", "2": "30", "3": "60", "4": "Original"}
    ...
    if merging_mode == "concat":
        ...
        layout = '|'.join([f'{(i % grid_cols) * width}_{(i // grid_cols) * height}' for i in range(num_inputs)])
        filters += f'{xstack_inputs} xstack=inputs={num_inputs}:layout={layout}[outv]'
