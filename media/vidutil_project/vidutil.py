...
def print_warning(message):
    print_formatted_text(HTML(f"<ansired>{message}</ansired>"), style=style)

def print_success(message):
    print_formatted_text(HTML(f"<ansigreen>{message}</ansigreen>"), style=style)

def print_info(message):
    print_formatted_text(HTML(f"<ansiyellow>{message}</ansiyellow>"), style=style)
...
def normalize_video(file, temp_dir, target_resolution, target_framerate=60.00, crf=15, preset='medium', tune='zerolatency'):
    if not verify_and_repair_video_format(file):
...
def merge_videos(output_file, input_files):
    ...
    if merging_mode == "concat":
        ...
        for input_file in valid_input_files:
            normalized_file = normalize_video(input_file, temp_dir, target_resolution, target_framerate)
            if normalized_file:
        ...
        try:
            ...
            logging.info(f"✔️ Video merging complete for: '{output_file}'.")
            print_success(f"✔️ Video merging complete for: '{output_file}'.")
        except subprocess.CalledProcessError as e:
            error_exit(f"Failed to concatenate videos. Error: {e.stderr.strip()}")
        ...
        try:
            subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=True)
            logging.info(f"✔️ Vertical stacking of videos completed for: '{output_file}'.")
            print_success(f"✔️ Vertical stacking of videos completed for: '{output_file}'.")
        except subprocess.CalledProcessError as e:
        ...
        try:
            subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=True)
            logging.info(f"✔️ Grid merging of videos completed for: '{output_file}'.")
            print_success(f"✔️ Grid merging of videos completed for: '{output_file}'.")
        except subprocess.CalledProcessError as e:
            error_exit(f"Failed to merge videos in a grid. Error: {e.stderr.strip()}")
        ...
        try:
            ...
            logging.info(f"✔️ Single video copied to '{output_file}'.")
            print_success(f"✔️ Single video copied to '{output_file}'.")
...
def screencaps_mode():
    ...
    try:
        ...
        error_exit(f"Failed to extract screenshots. Error: {e.stderr.strip()}")
...
def clip_mode():
    ...
    try:
        ...
        error_exit(f"Failed to create clip. Error: {e.stderr.strip()}")
