...
def run_stock() -> dict:
    ...
    composition = build_composition_matrix(skip=_ask_skip("COMPOSITION MATRIX"))
    composition["!EXEC_BIO_DERMAL_MAP"] = build_bio_dermal_map(skip=_ask_skip("BIO DERMAL MAP"))
    composition["!ENV_PHOTOMETRY"] = build_env_photometry(skip=_ask_skip("ENV PHOTOMETRY"))
    composition["!EXEC_MATERIAL_PHYSICS"] = build_material_physics(skip=_ask_skip("MATERIAL PHYSICS"))

    return {
        "!INIT_MEM_LOCK_PROTOCOL": build_mem_lock(),
        "!ENV_ATMOSPHERICS": build_env_atmospherics(skip=_ask_skip("ENV ATMOSPHERICS")),
        "!EXEC_COMPOSITION_MATRIX": composition,
        "TEXT_RECON_ENGINE": build_text_recon(skip=_ask_skip("TEXT RECON ENGINE")),
        "!FINALIZE_OUTPUT": build_finalize(),
    }
