...
is_tty() { [[ -t 1 ]]; }
GREEN=$(is_tty && printf '\033[0;32m' || echo '')
CYAN=$(is_tty && printf '\033[36m' || echo '')
RED=$(is_tty && printf '\033[0;31m' || echo '')
RESET=$(is_tty && printf '\033[0m' || echo '')
...
trap 'printf "%bAborted.%b\n" "$RED" "$RESET"; exit 130' SIGINT SIGTERM
...
run_ffmpeg() {
	...
	local filter_chain=""
	if [ "${#filters_arr[@]}" -gt 0 ]; then
		local IFS=,
		filter_chain="${filters_arr[*]}"
	fi
	...
	while true; do
		...
		11)
			...
			if [[ "$factor" =~ ^[0-9]+([.][0-9]+)?$ ]] && (($(echo "$factor > 0" | bc -l))); then
				...
				printf '%bInvalid factor. Must be > 0%b\n' "$RED" "$RESET"
			fi
			;;
		12)
			local factor
		...
		15)
			...
			if [ -n "$c" ] && [ -n "$r" ]; then
				...
				printf '%bCrop and scale required%b\n' "$RED" "$RESET"
			fi
			;;
		16)
			local deg
