...
def main():
    ...
    print(
        f"Checked out to new branch '{branch_name}' based on commit {selected_commit}."
    )

    # Ask user if they want to merge and push changes
    merge_choice = input(
        "Do you want to merge changes into your main branch and push to remote? (y/n): "
    ).lower()
    if merge_choice == "y":
        main_branch = input(
            "Enter the name of your main branch (e.g., 'main' or 'master'): "
        )
