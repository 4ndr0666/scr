...
backup_database() {
    backup_dir="/Nas/Backups/4ndr0man/current_db_backup"
    echo -e "\e[1;36mBacking up the current package database to $backup_dir...\e[0m"
    mkdir -p $backup_dir
    rsync -a /var/lib/pacman/local/ $backup_dir/
    if [ $? -eq 0 ]; then
        echo -e "\e[1;32mDatabase backup completed successfully.\e[0m"
    else
        echo -e "\e[1;31mDatabase backup failed.\e[0m"
        exit 1
    fi
}
...
# Main function
main() {
    echo -e "\e[1;36mBacking up the current package database...\e[0m"
    backup_database
