...
PKGLIST="https://raw.githubusercontent.com/4ndr0666/refs/heads/main/4ndr0tools/4ndr0pkglist.txt"
REPO_BRANCH="main"
AUR_HELPER="yay"
...
sleep 2
printf "%b\n" "Give me a moment while I set up the machine..."
sleep 1
...
setup_dotfiles() {
    ...
    sudo -u "$USER" rsync -a --exclude=".git/" "$HOME/dotfiles/" "$HOME/" || {
...
install_pkgs() {
    ...
    installpkg base-devel debugedit unzip archlinux-keyring github-cli git-delta exa lsd \
    fd micro expac bat bash-completion pacdiff pkgfile neovim xorg-xhost bat xclip ripgrep \
    diffuse neovim fzf sysz brave-beta-bin zsh-syntax-highlighting zsh-history-substring-search zsh-autosuggestions bashmount-git lsd lf-git || {
...
setupconfig() {    
    ...
    if [ -d "${HOME}/.config/Thunar/" ] && [ ! -d "${HOME}/.config/Thunar-bak" ]; then
        cp -r "${HOME}/.config/Thunar" "${HOME}/.config/Thunar-bak"
    fi
