...
DEFAULT_UNITS = [
    "wireplumber.service", "psd.service", "xdg-user-dirs-update.service",
    "p11-kit-server.socket", "gcr-ssh-agent.socket", "pipewire.socket",
    "gnome-keyring-daemon.socket", "pipewire-pulse.socket", "libinput-gestures.service",
    "pipewire-session-manager.service", "getty@tty1.service", "linux-modules-cleanup.service",
    "NetworkManager-wait-online.service", "btrfs-balance.timer", "btrfs-defrag.timer",
    "btrfs-scrub.timer", "btrfs-trim.timer", "snapper-cleanup.timer", "snapper-timeline.timer",
    "remote-fs.target", "garuda-pacman-lock.service", "garuda-pacman-snapshot-reject.service",
    "grub-btrfs-snapper.path", "ModemManager.service", "NetworkManager.service",
    "expressvpn.service", "ufw.service", "systemd-oomd.service", "cronie.service",
    "earlyoom.service", "avahi-daemon.service", "systemd-oomd.socket", "avahi-daemon.socket",
    "systemd-timesyncd.service", "haveged.service", "systemd-boot-check-no-failures.service",
    "dbus-org.freedesktop.ModemManager1.service", "dbus-org.freedesktop.nm-dispatcher.service",
    "dbus-org.freedesktop.timesync1.service", "display-manager.service", "default.target",
    "dbus-org.freedesktop.oom1.service", "earlyoom.service", "dbus-org.bluez.service",
    "dbus-org.freedesktop.Avahi.service", "proc-sys-fs-binfmt_misc.automount"
]
...
def compare_snapshots(default_snapshot, current_snapshot):
    ...
    diff = difflib.unified_diff(default_lines, current_lines, fromfile='default', tofile='current')
    return list(diff)
...
def align_services_with_presets():
    ...
    preset_units = run_command('systemctl list-unit-files --state=enabled,disabled')[0].split('\n')
...
def view_and_manage_snapshots():
    ...
    for i, snapshot in enumerate(snapshots, start=1):
        print(f"{i}. {snapshot}")
    choice = input("Select a snapshot to manage (type number) or 'R' to return: ").strip()
    if choice.lower() == 'r':
        return
...
def duplicate_snapshot(snapshot_name):
    ...
    if os.path.exists(original_path):
        ...
        logging.error("Attempted to duplicate a non-existent snapshot: " + snapshot_name)
...
def menu():
    ...
    while True:
        ...
        if user_input == '1':
            ...
            current_snapshot = '/mnt/data/SystemSnapshot_06:59 PM Friday | May 31, 2024.conf'
            ...
            if unwanted_units:
                ...
                for unit in unwanted_units:
                    print(unit)
                confirm = input("Do you want to disable and stop these units? (yes/no): ").strip().lower()
                if confirm == 'yes':
